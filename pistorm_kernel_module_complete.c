...
static long pistorm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case PISTORM_IOC_SETUP:
        ...
        // Set all data pins (D0-D15) as input initially
        u32 fsel0 = readl(pistorm_dev->gpio_base + GPFSEL0);
        u32 fsel1 = readl(pistorm_dev->gpio_base + GPFSEL1);
        
        // Configure pins 8-23 as inputs (D0-D15)
        for (int i = 8; i <= 23; i++) {
            int reg_idx = i / 10;
            int shift = (i % 10) * 3;
            if (reg_idx == 0) {
                fsel0 &= ~(7 << shift);
                fsel0 |= (0 << shift);  // Input mode
            } else if (reg_idx == 1) {
                fsel1 &= ~(7 << shift);
                fsel1 |= (0 << shift);  // Input mode
            }
        }
        
        // Configure address pins (A0-A7) as outputs
        for (int i = 2; i <= 9; i++) {
            int shift = (i % 10) * 3;
            fsel0 &= ~(7 << shift);
            fsel0 |= (1 << shift);  // Output mode
        }
        
        // Configure control pins (RESET, RD, WR, etc.) as outputs
        fsel0 &= ~(7 << (PIN_RESET * 3));
        fsel0 |= (1 << (PIN_RESET * 3));  // Output mode
        fsel0 &= ~(7 << (PIN_RD * 3));
        fsel0 |= (1 << (PIN_RD * 3));     // Output mode
        fsel0 &= ~(7 << (PIN_WR * 3));
        fsel0 |= (1 << (PIN_WR * 3));     // Output mode
        
        writel(fsel0, pistorm_dev->gpio_base + GPFSEL0);
        writel(fsel1, pistorm_dev->gpio_base + GPFSEL1);
        
        // Initialize clock
        // Enable 200MHz CLK output on GPIO4
        writel(0x5a000000 | (1 << 5), pistorm_dev->gpclk_base + (0x70 / 4));  // GP0_CTL
        udelay(10);
        while ((readl(pistorm_dev->gpclk_base + (0x70 / 4))) & (1 << 7));
        udelay(100);
        writel(0x5a000000 | (6 << 12), pistorm_dev->gpclk_base + (0x74 / 4));  // GP0_DIV (divider for 200MHz)
        udelay(10);
        writel(0x5a000000 | 5 | (1 << 4), pistorm_dev->gpclk_base + (0x70 / 4));  // GP0_CTL (enable, PLLC)
        udelay(10);
        while (!((readl(pistorm_dev->gpclk_base + (0x70 / 4))) & (1 << 7)));
        udelay(100);
        
        // Set GPIO4 to ALT0 (GPCLK0)
        u32 fsel1_val = readl(pistorm_dev->gpio_base + GPFSEL1);
        fsel1_val &= ~(7 << 12);  // Clear GPIO4 function
        fsel1_val |= (4 << 12);   // Set to alt function 0 (GPCLK0)
        writel(fsel1_val, pistorm_dev->gpio_base + GPFSEL1);
    ...
    case PISTORM_IOC_RESET_SM:
        mutex_lock(&pistorm_dev->lock);
        // Reset state machine: set INIT high, wait, then low
        u32 set_reg = readl(pistorm_dev->gpio_base + GPSET0);
        set_reg |= (1 << PIN_RESET);  // Set RESET high (inactive initially)
        writel(set_reg, pistorm_dev->gpio_base + GPSET0);
        udelay(1500);
        u32 clr_reg = readl(pistorm_dev->gpio_base + GPCLR0);
        clr_reg |= (1 << PIN_RESET);  // Set RESET low (active reset)
        writel(clr_reg, pistorm_dev->gpio_base + GPCLR0);
        udelay(100);
        set_reg = readl(pistorm_dev->gpio_base + GPSET0);
        set_reg |= (1 << PIN_RESET);  // Set RESET high again (reset complete)
        writel(set_reg, pistorm_dev->gpio_base + GPSET0);
        mutex_unlock(&pistorm_dev->lock);
        break;
    ...
    case PISTORM_IOC_PULSE_RESET:
        ...
        u32 clr_reg = readl(pistorm_dev->gpio_base + GPCLR0);
        clr_reg |= (1 << PIN_RESET);  // Reset low (active)
        writel(clr_reg, pistorm_dev->gpio_base + GPCLR0);
        udelay(100000);  // 100ms
        u32 set_reg = readl(pistorm_dev->gpio_base + GPSET0);
        set_reg |= (1 << PIN_RESET);  // Reset high (inactive)
        writel(set_reg, pistorm_dev->gpio_base + GPSET0);
        mutex_unlock(&pistorm_dev->lock);
        break;
    ...
    case PISTORM_IOC_BUSOP:
        ...
        if (busop.is_read) {
            ...
            if (busop.addr == 0xBADBAD00) {
                // Query current GPIO pin states (simulate *(gpio + 13) access)
                // This returns the GPLEV0 register value which contains pin states
                result = readl(pistorm_dev->gpio_base + GPLEV0);
                
                // For emulator compatibility, we need to return appropriate values
                // that indicate the state of TXN_IN_PROGRESS and IPL_ZERO pins
                // In a real implementation, we'd map the actual pin states
                ...
                // Set address lines
                u32 addr_low = (busop.addr & 0xFF) << 2;  // A0-A7 on pins 2-9
                u32 addr_high = ((busop.addr >> 8) & 0xFF) << 10;  // A8+ on pins 10+
                
                // Set address pins to output
                u32 fsel0 = readl(pistorm_dev->gpio_base + GPFSEL0);
                for (int i = 2; i <= 9; i++) {  // A0-A7
                    int shift = (i % 10) * 3;
                    fsel0 &= ~(7 << shift);
                    fsel0 |= (1 << shift);  // Output mode
                }
                ...
                u32 set_reg = readl(pistorm_dev->gpio_base + GPSET0);
                set_reg &= 0xFF000003;  // Clear address pins
                set_reg |= addr_low;
                writel(set_reg, pistorm_dev->gpio_base + GPSET0);
                
                // Set data pins to input mode
                fsel0 = readl(pistorm_dev->gpio_base + GPFSEL0);
                for (int i = 8; i <= 23; i++) {  // D0-D15
                    int shift = (i % 10) * 3;
                    fsel0 &= ~(7 << shift);
                    fsel0 |= (0 << shift);  // Input mode
                }
                ...
                // Pulse RD signal
                set_reg = readl(pistorm_dev->gpio_base + GPSET0);
                set_reg |= (1 << PIN_RD);  // RD high
                writel(set_reg, pistorm_dev->gpio_base + GPSET0);
                udelay(1);  // Setup time
                u32 clr_reg = readl(pistorm_dev->gpio_base + GPCLR0);
                clr_reg |= (1 << PIN_RD);  // RD low
                writel(clr_reg, pistorm_dev->gpio_base + GPCLR0);
                ...
                // Read data from pins
                u32 data_reg = readl(pistorm_dev->gpio_base + GPLEV0);
                result = (data_reg >> 8) & 0xFFFF;
            ...
            // Write operation
            // This is a simplified implementation - in reality, this would
            // perform the actual Amiga bus write operation
            ...
            // Write data to pins
            set_reg = readl(pistorm_dev->gpio_base + GPSET0);
            set_reg &= 0x000000FF;  // Clear data pins
            set_reg |= ((busop.value & 0xFFFF) << 8);  // Put data on D0-D15
            writel(set_reg, pistorm_dev->gpio_base + GPSET0);
            
            // Pulse WR signal
            set_reg = readl(pistorm_dev->gpio_base + GPSET0);
            set_reg |= (1 << PIN_WR);  // WR high
            writel(set_reg, pistorm_dev->gpio_base + GPSET0);
            udelay(1);  // Setup time
            u32 clr_reg = readl(pistorm_dev->gpio_base + GPCLR0);
            clr_reg |= (1 << PIN_WR);  // WR low
            writel(clr_reg, pistorm_dev->gpio_base + GPCLR0);
