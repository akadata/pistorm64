...
- The generated opinfo/translator path is still 68000-only, so non-68000 CPU models should default back to Musashi unless explicitly overridden for debugging.
- `PISTORM_M68XK_EXEC=1` is an experimental execution gate, not a production-ready fast path yet.
- Musashi-side 68040 baseline now includes MOVEC PCR read support (`reg 0x808`) for CPU-ID probe code paths. `PISTORM_PCR_ID` can override the exposed ID word when needed.
