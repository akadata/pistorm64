...
/* signed and unsigned int must be at least 32 bit */
...
/* Allow for architectures that don't have 32-bit sizes */
	...
	{
		return (sint)((value & 0x80000000) ? value | ~0xffffffff : value & 0xffffffff);
	}
...
typedef struct __attribute__((aligned(16))) m68ki_cpu_core
{
	...
	uint8 mmu_tmp_buserror_address;   /* temporary hack: (first) bus error address */
...
INLINE uint m68ki_read_imm_16(m68ki_cpu_core *state);
INLINE uint m68ki_read_imm_32(m68ki_cpu_core *state);
INLINE void m68ki_set_sp(m68ki_cpu_core *state, uint32_t pc);

/* Read data with specific function code */
INLINE uint m68ki_read_8_fc  (m68ki_cpu_core *state, uint address, uint fc);
INLINE uint m68ki_read_16_fc (m68ki_cpu_core *state, uint address, uint fc);
INLINE uint m68ki_read_32_fc (m68ki_cpu_core *state, uint address, uint fc);

/* Write data with specific function code */
INLINE void m68ki_write_8_fc (m68ki_cpu_core *state, uint address, uint fc, uint value);
INLINE void m68ki_write_16_fc(m68ki_cpu_core *state, uint address, uint fc, uint value);
INLINE void m68ki_write_32_fc(m68ki_cpu_core *state, uint address, uint fc, uint value);
...
INLINE void m68ki_set_sp(uint32_t pc)
{
	REG_SP = pc;
}
