...
{
	m68040_fpu_op0(&m68ki_cpu);
}


void m68k_op_040fpu1_32(void)
{
	m68040_fpu_op1(&m68ki_cpu);
}
...
void m68k_op_asr_8_s(void)
{
	...
	*r_dst = MASK_OUT_BELOW_8(*r_dst) | res;
	...
	FLAG_Z = res;
	FLAG_V = VFLAG_CLEAR;
	FLAG_X = (uint)(FLAG_C = src << (9-shift));
}
	...
	if(shift != 0)
	{
		...
		if(shift < 8)
		{
			...
			*r_dst = MASK_OUT_BELOW_8(*r_dst) | res;
			...
			FLAG_X = FLAG_C = (uint)(src << (9-shift));
			FLAG_N = NFLAG_8(res);
			FLAG_Z = res;
			FLAG_V = VFLAG_CLEAR;
			return;
...
void m68k_op_bsr_8(void)
{
	...
	m68k_enter_subroutine(REG_PC, REG_PC + MAKE_INT_8(MASK_OUT_ABOVE_8(REG_IR)));
	m68ki_branch_8(MASK_OUT_ABOVE_8(REG_IR));
}
...
void m68k_op_bsr_16(void)
{
	...
	m68ki_push_32(REG_PC);
	REG_PC -= 2;
	m68k_enter_subroutine(REG_PC, REG_PC + MAKE_INT_16(offset));
	m68ki_branch_16(offset);
}
...
void m68k_op_bsr_32(void)
{
	if(CPU_TYPE_IS_EC020_PLUS(CPU_TYPE))
	{
		...
		m68ki_push_32(REG_PC);
		REG_PC -= 4;
		m68k_enter_subroutine(REG_PC, REG_PC + offset);
		m68ki_branch_32(offset);
		return;
	}
	m68ki_exception_illegal();
