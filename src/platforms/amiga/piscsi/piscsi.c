...
static int piscsi_check_bounds(struct piscsi_dev *d, uint64_t offset, uint32_t len, int unit, const char *op)
{
    uint64_t end = offset + (uint64_t)len;
    if (end > d->fs) {
        DEBUG("[PISCSI-IO-ERROR] Unit:%d %s beyond end of disk: off=0x%llx len=%u size=0x%llx\n",
              unit, op, (unsigned long long)offset, len, (unsigned long long)d->fs);
        return -1;
    }
    return 0;
}
    ...
    switch (cmd) {
        ...
        case PISCSI_CMD_READBYTES:
            ...
            if (cmd == PISCSI_CMD_READBYTES) {
                ...
                if (piscsi_check_bounds(d, src, piscsi_u32[1], val, "READ64") != 0)
                    break;
                lseek64(d->fd, src, SEEK_SET);
            }
            ...
            if (map) {
                ...
                ssize_t bytes_read = read(d->fd, d->scratch, piscsi_u32[1]);
                if (bytes_read <= 0) {
        ...
        case PISCSI_CMD_WRITEBYTES:
            ...
            if (cmd == PISCSI_CMD_WRITEBYTES) {
                ...
                if (piscsi_check_bounds(d, src, piscsi_u32[1], val, "WRITE64") != 0)
                    break;
                lseek64(d->fd, src, SEEK_SET);
            }
