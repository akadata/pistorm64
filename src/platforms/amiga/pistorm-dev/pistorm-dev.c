...
static int32_t grab_amiga_string(uint32_t addr, uint8_t* dest, uint32_t str_max_len) {
  ...
  if(r == -1) {
    DEBUG("[GRAB_AMIGA_STRING] No mapped range found for address $%.8X. Grabbing string data over "
          "the bus.\n",
          addr);
    do {
...
static int32_t amiga_transfer_file(uint32_t addr, char* filename) {
  ...
  fseek(in, 0, SEEK_SET);
  if(r == -1) {
    DEBUG("[GRAB_AMIGA_STRING] No mapped range found for address $%.8X. Transferring file data "
          "over the bus.\n",
          addr);
    uint8_t tmp_read = 0;
...
void handle_pistorm_dev_write(uint32_t addr_, uint32_t val, uint8_t type) {
  ...
  case PI_CMD_TRANSFERFILE:
    ...
    if(pi_string[0] == 0 || grab_amiga_string(pi_string[0], (uint8_t*)tmp_string, 255) == -1) {
      ...
      if(amiga_transfer_file(pi_ptr[0], tmp_string) == -1) {
        pi_cmd_result = (uint8_t)PI_RES_FAILED;
      } else {
        pi_cmd_result = (uint8_t)PI_RES_OK;
      }
    }
        ...
        for(int y = 0; y < pi_word[3] && y + pi_word[4] < pi_word[5]; y++) {
          for(int x = 0; x < pi_word[2]; x++) {
            ...
            if(color && y + pi_word[4] >= 0) {
              *dst_ptr = (pal_ptr) ? pal_ptr[color] : (uint8_t)color;
            }
            dst_ptr++;
            tmp <<= val;
            num_bits -= val;
  ...
  case PI_CMD_RTG_STATUS:
    ...
    if(val == 1 && !rtg_enabled) {
      ...
      if(!rtg_on) {
        ...
        // Refuse to disable PIRTG64 if it's currently in use.
        pi_cmd_result = (uint8_t)PI_RES_FAILED;
      }
    } else {
  ...
  case PI_CMD_SET_CLUT_CURSOR: {
    ...
    rtg_set_clut_cursor(bmp, (uint32_t*)(void*)pal, (int16_t)pi_word[0], (int16_t)pi_word[1], (uint8_t)pi_word[2],
                        (uint8_t)pi_word[3], (uint8_t)pi_word[4]);
    break;
  }
    ...
    if(pi_string[0] == 0 || grab_amiga_string(pi_string[0], (uint8_t*)tmp_string, 255) == -1) {
      ...
      if(tmp == NULL) {
        ...
        if(get_named_mapped_item(cfg, "kickstart") != -1) {
          ...
          add_mapping(cfg, MAPTYPE_ROM, (uint32_t)cfg->map_offset[index], (uint32_t)cfg->map_size[index], 0, tmp_string,
                      "kickstart", 0);
          pi_cmd_result = (uint8_t)PI_RES_OK;
          do_reset = 1;
        } else {
  ...
  case PI_CMD_CONFIRMSHUTDOWN:
    if(val != shutdown_confirm) {
      DEBUG("[PISTORM-DEV] Attempted shutdown with wrong shutdown confirm value. Not shutting "
            "down.\n");
  ...
  default:
    DEBUG("[PISTORM-DEV] WARN: Unhandled %s register write to %.4X: %d\n", op_type_names[type],
          addr - pistorm_dev_base, val);
    pi_cmd_result = (uint8_t)PI_RES_INVALIDCMD;
    break;
  }
}
...
uint32_t handle_pistorm_dev_read(uint32_t addr_, uint8_t type) {
  ...
  case PI_DBG_VAL8:
    DEBUG("[PISTORM-DEV] Read DEBUG VALUE %d (%d / $%.8X)\n", (addr - PI_DBG_VAL1) / 4,
          pi_dbg_val[(addr - PI_DBG_VAL1) / 4], pi_dbg_val[(addr - PI_DBG_VAL1) / 4]);
  ...
  case PI_BYTE7:
  case PI_BYTE8:
    DEBUG("[PISTORM-DEV] Read BYTE %d (%d / $%.2X)\n", addr - PI_BYTE1, pi_byte[addr - PI_BYTE1],
          pi_byte[addr - PI_BYTE1]);
  ...
  case PI_WORD4:
    DEBUG("[PISTORM-DEV] Read WORD %d (%d / $%.4X)\n", (addr - PI_WORD1) / 2,
          pi_word[(addr - PI_WORD1) / 2], pi_word[(addr - PI_WORD1) / 2]);
  ...
  case PI_WORD12:
    DEBUG("[PISTORM-DEV] Read WORD %d (%d / $%.4X)\n", (addr - PI_WORD5) / 2,
          pi_word[(addr - PI_WORD5) / 2], pi_word[(addr - PI_WORD5) / 2]);
  ...
  case PI_LONGWORD4:
    DEBUG("[PISTORM-DEV] Read LONGWORD %d (%d / $%.8X)\n", (addr - PI_LONGWORD1) / 4,
          pi_longword[(addr - PI_LONGWORD1) / 4], pi_longword[(addr - PI_LONGWORD1) / 4]);
