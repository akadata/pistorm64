...
static inline uint8_t* rtg_pixel_at(
  uint8_t *base, 
  size_t index, 
  uint16_t format
  ) {
...
static inline uint32_t rtg_load_pixel(
  const uint8_t *dest, 
  uint16_t format
  ) {
  switch (format) {
...
static inline void rtg_store_pixel(
  uint8_t *dest, 
  uint16_t format, 
  uint32_t value
  ) {
...
static inline void rtg_store_pixel_mask(
  uint8_t *dest, 
  uint16_t format, 
  uint32_t value,                                        
  uint32_t mask
...
static inline void rtg_invert_pixel(
  uint8_t *dest,   
  uint16_t format, 
  uint32_t mask
  ) {
...
void rtg_set_scale_rect(uint16_t scale_mode, 
  int16_t x1, 
  int16_t y1, 
  int16_t x2, 
  int16_t y2);
...
void rtg_set_clut_cursor(uint8_t* bmp, 
  uint32_t* pal, 
  int16_t offs_x, 
  int16_t offs_y, 
  uint16_t w,                         
