    ...
    switch (cmd) {
        ...
        case PISCSI64_CMD_READBYTES:
            ...
            if (map_rc_read == 0) {
                ...
                for (uint32_t i = 0; i < piscsi64_u32[1]; i++) {
                    ...
                    m68k_write_memory_8(piscsi64_u32[2] + i, (uint32_t)c);
                }
                if (success) {
                    DEBUG("[PISCSI64-IO-SUCCESS] Unit:%d BYTE READ: %d bytes OK\n", val, piscsi64_u32[1]);
                }
            }
            break;
        ...
        case PISCSI64_CMD_WRITEBYTES:
            ...
            if (map_rc_write == 0) {
                ...
                if (success) {
                    DEBUG("[PISCSI64-IO-SUCCESS] Unit:%d BYTE WRITE: %d bytes OK\n", val, piscsi64_u32[1]);
                }
            }
            break;
...
fs_found:;
        ...
        case PISCSI64_CMD_LOADFS: {
            ...
            if (mapped_r != -1) {
                ...
                piscsi64_filesystems[piscsi64_num_fs].FS_ID = piscsi64_rom_partition_dostype[piscsi64_rom_cur_partition];
