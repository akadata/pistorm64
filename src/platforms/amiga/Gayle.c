...
uint8_t ide_feature_w = 0, ide_command_w = 0, ide_sec_count = 0, ide_sec_num = 0, idewrite8 = 0,
        ide_cyl_hi = 0, ide_dev_head = 0;
uint8_t ide_devctrl_w = 0, ide_cyl_low = 0, ide_error_r = 0, ide_status_r = 0, ide_altst_r = 0,
        ide_data = 0;
  ...
  if ((address & GAYLEMASK) == CLOCKBASE) {
    if ((address & CLOCKMASK) >= 0x8000) {
      if (cdtv_mode) {
        ...
        // address, cdtv_sram[(address & CLOCKMASK) - 0x8000]);
