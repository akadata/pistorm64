 ...
 *   LICENSE: zlib/libpng
 *
 *   Copyright (c) 2019 Kevin 'Assyrianic' Yonan (@assyrianic) and reviewed by Ramon Santamaria
 *(@raysan5)
 *
...
static void __InsertMemNodeBefore(AllocList* const list, MemNode* const insert,
                                  MemNode* const curr) {
  insert->next = curr;
  if (curr->prev == NULL)
    list->head = insert;
...
static void __InsertMemNode(MemPool* const mempool, AllocList* const list, MemNode* const node,
  ...
  // NULL ptr should make this work like regular Allocation.
    ...
    if (resized_block == NULL)
      ...
      memmove(resized_block, ptr,
              (node->size > resized->size) ? (resized->size - NODE_SIZE)
                                           : (node->size - NODE_SIZE));
...
size_t GetMemPoolFreeMemory(const MemPool mempool) {
  ...
  for (MemNode* n = mempool.large.head; n != NULL; n = n->next)
    total_remaining += n->size;

  for (size_t i = 0; i < MEMPOOL_BUCKET_SIZE; i++)
    for (MemNode* n = mempool.buckets[i].head; n != NULL; n = n->next)
      total_remaining += n->size;
...
ObjPool CreateObjPoolFromBuffer(void* const restrict buf, const size_t objsize, const size_t len) {
  ...
  const size_t aligned_size = __AlignSize(objsize, sizeof(size_t));
  if ((buf == NULL) || (len == 0) || (objsize < sizeof(size_t)) ||
      (objsize * len != aligned_size * len))
    return objpool;
  else {
    objpool.objSize = aligned_size;
    objpool.memSize = objpool.freeBlocks = len;
...
void ObjPoolFree(ObjPool* const restrict objpool, void* const ptr) {
  ...
  if ((ptr == NULL) || (block < objpool->mem) ||
      (block > objpool->mem + objpool->memSize * objpool->objSize))
    return;
  else {
    // When we free our pointer, we recycle the pointer space to store the previous index and then
    // we push it as our new head. *p = index of Head in relation to the buffer; Head = p;
    size_t* const restrict index = (size_t*)block;
    *index =
        (objpool->offs != 0) ? (objpool->offs - objpool->mem) / objpool->objSize : objpool->memSize;
    objpool->offs = block;
    objpool->freeBlocks++;
  }
}
