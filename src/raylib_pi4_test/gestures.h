...
void ProcessGestureEvent(
    GestureEvent event);   // Process gesture event and translate it into gestures
...
#define MINIMUM_DRAG                                                                               \
  0.015f // Drag minimum force, measured in normalized screen units (0.0f to 1.0f)
#define MINIMUM_PINCH                                                                              \
  0.005f                  // Pinch minimum force, measured in normalized screen units (0.0f to 1.0f)
...
void ProcessGestureEvent(GestureEvent event) {
  ...
  if (GESTURES.Touch.pointCount < 2) {
    if (event.touchAction == TOUCH_DOWN) {
      ...
      if ((GESTURES.current == GESTURE_NONE) && (GESTURES.Touch.tapCounter >= 2) &&
          ((GetCurrentTime() - GESTURES.Touch.eventTime) < TAP_TIMEOUT) &&
          (Vector2Distance(GESTURES.Touch.downPositionA, event.position[0]) < DOUBLETAP_RANGE)) {
      ...
      GESTURES.Drag.distance =
          Vector2Distance(GESTURES.Touch.downPositionA, GESTURES.Touch.upPosition);
      GESTURES.Drag.intensity =
          GESTURES.Drag.distance / (float)((GetCurrentTime() - GESTURES.Swipe.timeDuration));
      ...
      if ((GESTURES.Drag.intensity > FORCE_TO_SWIPE) &&
        ...
        GESTURES.Drag.angle =
            360.0f - Vector2Angle(GESTURES.Touch.downPositionA, GESTURES.Touch.upPosition);
      ...
      if (GESTURES.current == GESTURE_HOLD) {
        ...
        // Detect GESTURE_DRAG
        if (Vector2Distance(GESTURES.Touch.downPositionA, GESTURES.Touch.moveDownPositionA) >=
            MINIMUM_DRAG) {
          GESTURES.Touch.eventTime = GetCurrentTime();
  ...
  } else // Two touch points
  {
    if (event.touchAction == TOUCH_DOWN) {
      ...
      // GESTURES.Pinch.distance = Vector2Distance(GESTURES.Touch.downPositionA,
      // GESTURES.Touch.downPositionB);
      ...
      GESTURES.Pinch.vector.x =
          GESTURES.Touch.moveDownPositionB.x - GESTURES.Touch.moveDownPositionA.x;
      GESTURES.Pinch.vector.y =
          GESTURES.Touch.moveDownPositionB.y - GESTURES.Touch.moveDownPositionA.y;

      if ((Vector2Distance(GESTURES.Touch.downPositionA, GESTURES.Touch.moveDownPositionA) >=
           MINIMUM_PINCH) ||
          (Vector2Distance(GESTURES.Touch.downPositionB, GESTURES.Touch.moveDownPositionB) >=
           MINIMUM_PINCH)) {
        if ((Vector2Distance(GESTURES.Touch.moveDownPositionA, GESTURES.Touch.moveDownPositionB) -
      ...
      GESTURES.Pinch.angle =
          360.0f - Vector2Angle(GESTURES.Touch.moveDownPositionA, GESTURES.Touch.moveDownPositionB);
    } else if (event.touchAction == TOUCH_UP) {
...
void UpdateGestures(void) {
  ...
  if (((GetCurrentTime() - GESTURES.Touch.eventTime) > TAP_TIMEOUT) &&
      (GESTURES.current == GESTURE_DRAG) && (GESTURES.Touch.pointCount < 2)) {
    GESTURES.current = GESTURE_HOLD;
    GESTURES.Hold.timeDuration = GetCurrentTime();
...
float GetGestureHoldDuration(void) {
  ...
  if (GESTURES.current == GESTURE_HOLD)
    time = GetCurrentTime() - GESTURES.Hold.timeDuration;
...
Vector2 GetGesturePinchVector(void) {
  // NOTE: The position values used for GESTURES.Pinch.distance are not modified like the position
  // values of [core.c]-->GetTouchPosition(int index) NOTE: pinch distance is calculated on two
  // touch points TOUCH_MOVE
...
static double GetCurrentTime(void) {
  ...
  unsigned long long int nowTime = (unsigned long long int)now.tv_sec * 1000000000LLU +
                                   (unsigned long long int)now.tv_nsec; // Time in nanoseconds
