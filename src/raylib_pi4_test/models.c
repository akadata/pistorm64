...
#define GL_TEXTURE_COORD_ARRAY 0x8078
  ...
  //    transform: model transformation provided (includes DrawModel() params combined with
...
bool ExportMesh(Mesh mesh, const char* fileName) {
  ...
  if (IsFileExtension(fileName, ".obj")) {
    ...
    for (int i = 0, v = 0; i < mesh.vertexCount; i++, v += 2) {
      byteCount +=
          sprintf(txtData + byteCount, "vt %.3f %.3f\n", mesh.texcoords[v], mesh.texcoords[v + 1]);
    }
    ...
    if (data->nodes[nodeId].mesh == &(data->meshes[primitiveIndex])) {
      if (model->meshes[primitiveIndex].boneIds == NULL) {
        model->meshes[primitiveIndex].boneIds =
            RL_CALLOC(model->meshes[primitiveIndex].vertexCount * 4, sizeof(int));
        model->meshes[primitiveIndex].boneWeights =
            RL_CALLOC(model->meshes[primitiveIndex].vertexCount * 4, sizeof(float));
...
void LoadGLTFNode(cgltf_data* data, cgltf_node* node, Model* outModel, Matrix currentTransform,
                  int* primitiveIndex, const char* fileName) {
  // Apply the transforms if they exist (Will still be applied even if no mesh is present to support
  // emptys and bone structures)
  ...
  if (node->mesh != NULL) {
    ...
    if ((node->skin != NULL) && (node->parent != NULL)) {
      ...
      TRACELOG(LOG_WARNING,
               "MODEL: GLTF Node %s is skinned but not root node! Parent transformations will be "
               "ignored (NODE_SKINNED_MESH_NON_ROOT)",
               node->name);
    }
...
static Model LoadVOX(const char* fileName) {
  ...
  if (ret != VOX_SUCCESS) {
    ...
    TRACELOG(LOG_INFO, "MODEL: [%s] VOX data loaded successfully : %i vertices/%i meshes", fileName,
             nbvertices, meshescount);
  }
