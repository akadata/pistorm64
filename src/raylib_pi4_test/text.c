...
Font LoadFontFromMemory(const char* fileType, const unsigned char* fileData, int dataSize,
  ...
  if (TextIsEqual(fileExtLower, ".ttf") || TextIsEqual(fileExtLower, ".otf")) {
    ...
    font.glyphPadding = 0;
    font.glyphs =
        LoadFontData(fileData, dataSize, font.baseSize, fontChars, font.glyphCount, FONT_DEFAULT);
...
#if defined(SUPPORT_FILEFORMAT_TTF)
Image GenImageFontAtlas(const GlyphInfo* chars, Rectangle** charRecs, int glyphCount, int fontSize,
...
void DrawTextPro(Font font, const char* text, Vector2 position, Vector2 origin, float rotation,
...
void DrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint) {
  ...
  Rectangle dstRec = {position.x + font.glyphs[index].offsetX * scaleFactor -
...
#define MAX_BUFFER_SIZE 256
  ...
  if (font.texture.id == 0) {
    ...
    TRACELOG(LOG_INFO, "FONT: [%s] Font loaded successfully (%i glyphs)", fileName,
             font.glyphCount);
