...
floatx80 floatx80_acos(floatx80 a, float_status* status) {
  ...
  if (aExp == 0 && aSig == 0) {
    ...
    return roundAndPackFloatx80(status->floatx80_rounding_precision, 0, piby2_exp, pi_sig, 0,
                                status);
  }
  ...
  if (compact < 0x3FFB8000 || compact > 0x4002FFFF) { // |X| >= 16 or |X| < 1/16
    if (compact > 0x3FFF8000) {                       // |X| >= 16
      if (compact > 0x40638000) {                     // |X| > 2^(100)
        ...
        fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0xBFC24924827107B8), status),
                           status); // C3+Z*C5
        fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3FC999999996263E), status),
                           status);           // C2+Z*C4
        fp1 = floatx80_mul(fp1, fp3, status); // Z*(C3+Z*C5)
        fp2 = floatx80_mul(fp2, fp0, status); // Y*(C2+Z*C4)
        fp1 = floatx80_add(fp1, float64_to_floatx80(LIT64(0xBFD5555555555536), status),
                           status);             // C1+Z*(C3+Z*C5)
      ...
      if (compact < 0x3FD78000) { // |X| < 2^(-40)
        ...
        fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3FBC71C646940220), status),
                           status); // B4+Z*B6
        fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0xBFC24924921872F9), status),
                           status);           // B3+Z*B5
        fp2 = floatx80_mul(fp2, fp1, status); // Z*(B4+Z*B6)
        fp1 = floatx80_mul(fp1, fp3, status); // Z*(B3+Z*B5)
        fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3FC9999999998FA9), status),
                           status); // B2+Z*(B4+Z*B6)
        fp1 = floatx80_add(fp1, float64_to_floatx80(LIT64(0xBFD5555555555555), status),
                           status);             // B1+Z*(B3+Z*B5)
    ...
    fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x4002AC6934A26DB3), status),
                       status); // A2+V*(A3+V)
    fp1 =
        floatx80_mul(fp1, float64_to_floatx80(LIT64(0xBFC2476F4E1DA28E), status), status); // A1+U*V
...
floatx80 floatx80_cos(floatx80 a, float_status* status) {
  ...
  if (compact < 0x3FD78000 || compact > 0x4004BC7E) { // 2^(-40) > |X| > 15 PI
    ...
    loop:
      ...
      fp2 = floatx80_add(fp2, float32_to_floatx80(twoto63, status),
                         status); // THE FRACTIONAL PART OF FP2 IS ROUNDED
    ...
    fp1 =
        floatx80_mul(fp0, float64_to_floatx80(LIT64(0x3FE45F306DC9C883), status), status); // X*2/PI
  ...
  sincont:
    if ((flag)((n + adjn) & 1)) {
      ...
      fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3E21EED90612C972), status),
                         status); // B6+TB8
      fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0xBE927E4FB79D9FCF), status),
                         status);           // B5+TB7
      fp2 = floatx80_mul(fp2, fp1, status); // T(B6+TB8)
      fp3 = floatx80_mul(fp3, fp1, status); // T(B5+TB7)
      fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3EFA01A01A01D423), status),
                         status); // B4+T(B6+TB8)
      ...
      fp1 =
          floatx80_add(fp1, float32_to_floatx80(0xBF000000, status), status); // B1+T(B3+T(B5+TB7))
      ...
      fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0xBE5AE6452A118AE4), status),
                         status); // A5+T*A7
      fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3EC71DE3A5341531), status),
                         status);           // A4+T*A6
      fp3 = floatx80_mul(fp3, fp1, status); // T(A5+TA7)
      fp2 = floatx80_mul(fp2, fp1, status); // T(A4+TA6)
      fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0xBF2A01A01A018B59), status),
                         status); // A3+T(A5+TA7)
...
floatx80 floatx80_etox(floatx80 a, float_status* status) {
  ...
  if (aExp >= 0x3FBE) { // |X| >= 2^(-65)
    ...
    expcont1:
      fp2 = fp0; // N
      fp0 = floatx80_mul(fp0, float32_to_floatx80(0xBC317218, status),
      ...
      fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3FA5555555554431), status),
                         status); // fp2 is A3+S*A5
      fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0x3FC5555555554018), status),
                         status);           // fp3 is A2+S*A4
      fp2 = floatx80_mul(fp2, fp1, status); // fp2 is S*(A3+S*A5)
      fp3 = floatx80_mul(fp3, fp1, status); // fp3 is S*(A2+S*A4)
      fp2 = floatx80_add(fp2, float32_to_floatx80(0x3F000000, status),
      ...
      fp0 =
          floatx80_add(fp0, float32_to_floatx80(exp_tbl2[j], status), status); // accurate 2^(J/64)
      ...
      if (compact > 0x400CB27C) { // |X| >= 16480 log2
        RESET_PREC;
        if (aSign) {
          a = roundAndPackFloatx80(status->floatx80_rounding_precision, 0, -0x1000, aSig, 0,
                                   status);
        } else {
        ...
        if (k < 0 &&
            (k & 1)) { // arithmetic right shift is division and round towards minus infinity
          m1--;
        }
        m = k - m1;
...
floatx80 floatx80_etoxm1(floatx80 a, float_status* status) {
  ...
  if (aExp >= 0x3FFD) { // |X| >= 1/4
    ...
    if (compact <= 0x4004C215) { // |X| <= 70 log2
      ...
      fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3F81111111174385), status),
                         status); // fp2 IS A4+S*A6
      fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0x3FA5555555554F5A), status),
                         status);           // fp3 is A3+S*A5
      fp2 = floatx80_mul(fp2, fp1, status); // fp2 IS S*(A4+S*A6)
      fp3 = floatx80_mul(fp3, fp1, status); // fp3 IS S*(A3+S*A5)
      fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3FC5555555555555), status),
                         status); // fp2 IS A2+S*(A4+S*A6)
      fp3 = floatx80_add(fp3, float32_to_floatx80(0x3F000000, status),
...
floatx80 floatx80_logn(floatx80 a, float_status* status) {
  ...
  if (compact < 0x3FFEF07D || compact > 0x3FFF8841) { // |X| < 15/16 or |X| > 17/16
    ...
    fp1 = floatx80_add(fp1, float64_to_floatx80(LIT64(0x3FC99999987D8730), status),
                       status); // A4+V*A6
    fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0xBFCFFFFFFF6F7E97), status),
                       status);           // A3+V*A5
    fp1 = floatx80_mul(fp1, fp3, status); // V*(A4+V*A6)
    fp2 = floatx80_mul(fp2, fp3, status); // V*(A3+V*A5)
    fp1 = floatx80_add(fp1, float64_to_floatx80(LIT64(0x3FD55555555555A4), status),
                       status); // A2+V*(A4+V*A6)
    fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0xBFE0000000000008), status),
                       status);           // A1+V*(A3+V*A5)
    ...
    fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0x3F624924928BCCFF), status),
                       status); // B3+W*B5
    fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3F899999999995EC), status),
                       status);           // B2+W*B4
    fp1 = floatx80_mul(fp1, fp3, status); // W*(B3+W*B5)
    fp2 = floatx80_mul(fp2, fp0, status); // V*(B2+W*B4)
    fp1 = floatx80_add(fp1, float64_to_floatx80(LIT64(0x3FB5555555555555), status),
                       status); // B1+W*(B3+W*B5)
...
floatx80 floatx80_sinh(floatx80 a, float_status* status) {
  ...
  if (compact > 0x400CB167) {
    ...
    if (compact > 0x400CB2B3) {
      ...
      fp0 = floatx80_sub(fp0, float64_to_floatx80(LIT64(0x40C62D38D3D64634), status),
                         status); // (|X|-16381LOG2_LEAD)
      fp0 = floatx80_sub(fp0, float64_to_floatx80(LIT64(0x3D6F90AEB1E75CC7), status),
...
floatx80 floatx80_tan(floatx80 a, float_status* status) {
  ...
  tancont:
    if (n & 1) {
      ...
      fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0xBF346F59B39BA65F), status),
                         status); // Q3+SQ4
      ...
      fp0 = floatx80_add(fp0, float32_to_floatx80(0x3F800000, status),
      ...
      fp1 = floatx80_add(fp1, float32_to_floatx80(0x3F800000, status),
                         status); // 1+S(Q1+S(Q2+S(Q3+SQ4)))
  ...
  if (compact < 0x3FD78000 || compact > 0x3FFFDDCE) {
    ...
    if (compact < 0x3FFF8000) {
      ...
      if (compact > 0x40048AA1) {
        ...
        sign = aSign ? 0x80000000 : 0x00000000;
        fp1 = floatx80_div(float32_to_floatx80(sign ^ 0xC0000000, status), fp0,
...
floatx80 floatx80_tentox(floatx80 a, float_status* status) {
  ...
  if (compact < 0x3FB98000 || compact > 0x400B9B07) { // |X| > 16480 LOG2/LOG10 or |X| < 2^(-70)
    if (compact > 0x3FFF8000) {                       // |X| > 16480
      ...
      if (aSign) {
        return roundAndPackFloatx80(status->floatx80_rounding_precision, 0, -0x1000, aSig, 0,
                                    status);
      } else {
        return roundAndPackFloatx80(status->floatx80_rounding_precision, 0, 0x8000, aSig, 0,
                                    status);
      }
    } else { // |X| < 2^(-70)
      RESET_PREC;
    ...
    fp1 = fp0; // X
    fp1 = floatx80_mul(fp1, float64_to_floatx80(LIT64(0x406A934F0979A371), status),
                       status);         // X*64*LOG10/LOG2
    ...
    fp2 = fp1; // N
    fp1 = floatx80_mul(fp1, float64_to_floatx80(LIT64(0x3F734413509F8000), status),
    ...
    fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3FA5555555554CC1), status),
                       status); // A3+S*A5
    fp3 = floatx80_add(fp3, float64_to_floatx80(LIT64(0x3FC5555555554A54), status),
                       status);           // A2+S*A4
    fp2 = floatx80_mul(fp2, fp1, status); // S*(A3+S*A5)
    fp3 = floatx80_mul(fp3, fp1, status); // S*(A2+S*A4)
    fp2 = floatx80_add(fp2, float64_to_floatx80(LIT64(0x3FE0000000000000), status),
                       status);           // A1+S*(A3+S*A5)
