 ...
 *OpenBSD, NetBSD, DragonFly NOTE: Oculus Rift CV1 requires PLATFORM_DESKTOP for mirror rendering -
 *View [rlgl] module to enable it
 *
 *   #define PLATFORM_ANDROID
 *       Windowing and input system configured for Android device, app activity managed internally
 *in this module. NOTE: OpenGL ES 2.0 is required and graphic device is managed by EGL
 *
 *   #define PLATFORM_RPI
 *       Windowing and input system configured for Raspberry Pi in native mode (no XWindow
 *required), graphic device is managed by EGL and inputs are processed is raw mode, reading from
 * /dev/input/
 *
 *   #define PLATFORM_DRM
 *       Windowing and input system configured for DRM native mode (RPI4 and other devices)
 *       graphic device is managed by EGL and inputs are processed is raw mode, reading from
 * /dev/input/
 *
 *   #define PLATFORM_WEB
 *       Windowing and input system configured for HTML5 (run on browser), code converted from C to
 *asm.js using emscripten compiler. OpenGL ES 2.0 required for direct translation to WebGL
 *equivalent code.
 *
 *   #define SUPPORT_DEFAULT_FONT (default)
 *       Default font is loaded on window initialization to be available for the user to render
 *simple text. NOTE: If enabled, uses external module functions to load default raylib font (module:
 *text)
 *
 *   #define SUPPORT_CAMERA_SYSTEM
 *       Camera module is included (camera.h) and multiple predefined cameras are available: free,
 *1st/3rd person, orbital
 *
 *   #define SUPPORT_GESTURES_SYSTEM
 *       Gestures module is included (gestures.h) to support gestures detection: tap, hold, swipe,
 *drag
 *
 ...
 *running processes or blocking the device if not restored properly. Use with care.
 *
 *   #define SUPPORT_MOUSE_CURSOR_POINT
 *       Draw a mouse pointer on screen
 *
 *   #define SUPPORT_BUSY_WAIT_LOOP
 *       Use busy wait loop for timing sync, if not defined, a high-resolution timer is setup and
 *used
 *
 *   #define SUPPORT_PARTIALBUSY_WAIT_LOOP
 *       Use a partial-busy wait loop, in this case frame sleeps for most of the time and runs a
 *busy-wait-loop at the end
 *
 *   #define SUPPORT_EVENTS_WAITING
 *       Wait for events passively (sleeping while no events) instead of polling them actively every
 *frame
 *
 ...
 *       Support CompressData() and DecompressData() functions, those functions use zlib
 *implementation provided by stb_image and stb_image_write libraries, so, those libraries must be
 *enabled on textures module for linkage
 *
 *   #define SUPPORT_DATA_STORAGE
 *       Support saving binary data automatically to a generated storage.data file. This file is
 *managed internally
 *
 ...
 *
 *   DEPENDENCIES:
 *       rglfw    - Manage graphic device, OpenGL context and inputs on PLATFORM_DESKTOP (Windows,
 *Linux, OSX. FreeBSD, OpenBSD, NetBSD, DragonFly) raymath  - 3D math functionality (Vector2,
 *Vector3, Matrix, Quaternion) camera   - Multiple 3D camera modes (free, orbital, 1st person, 3rd
 *person) gestures - Gestures system for touch-ready devices (or simulated from mouse inputs)
 *
 *
 *   LICENSE: zlib/libpng
 *
...
//Font type #define GLFW_EXPOSE_NATIVE_WAYLAND #define GLFW_EXPOSE_NATIVE_MIR
...
//----------------------------------------------------------------------------------
#if defined(PLATFORM_RPI) || defined(PLATFORM_DRM)
#define USE_LAST_TOUCH_DEVICE // When multiple touchscreens are connected, only use the one with the
                              // highest event<N> number

#define DEFAULT_GAMEPAD_DEV                                                                        \
  "/dev/input/js"                        // Gamepad input (base dev for all gamepads: js0, js1, ...)
...
#if defined(PLATFORM_RPI) || defined(PLATFORM_DRM)
      char currentButtonStateEvdev[MAX_MOUSE_BUTTONS]; // Holds the new mouse state for the next
                                                       // polling event to grab (Can't be written
                                                       // directly due to multithreading, app could
                                                       // miss the update)
#endif
    } Mouse;
    struct {
...
static void
WindowSizeCallback(GLFWwindow* window, int width,
...
static void WindowIconifyCallback(
    GLFWwindow* window,
    int iconified); // GLFW3 WindowIconify Callback, runs when window is minimized/restored
static void
WindowFocusCallback(GLFWwindow* window,
...
static void
CursorEnterCallback(GLFWwindow* window,
...
static int32_t AndroidInputCallback(struct android_app* app,
                                    AInputEvent* event); // Process Android inputs
#endif

#if defined(PLATFORM_WEB)
static EM_BOOL EmscriptenTouchCallback(int eventType, const EmscriptenTouchEvent* touchEvent,
                                       void* userData);
...
static void ConfigureEvdevDevice(
    char* device); // Identifies a input device and configures it for use if appropriate
...
static void
ExportAutomationEvents(const char* fileName); // Export recorded automation events into a file
static void
RecordAutomationEvent(unsigned int frame); // Record frame events (to internal events array)
static void
PlayAutomationEvent(unsigned int frame); // Play frame events (from internal events array)
#endif

#if defined(_WIN32)
// NOTE: We declare Sleep() function symbol to avoid including windows.h (kernel32.lib linkage
// required)
...
void InitWindow(int width, int height, const char* title) {
  ...
  InitAssetManager(CORE.Android.app->activity->assetManager,
                   CORE.Android.app->activity->internalDataPath);
  ...
  while (!CORE.Window.ready) {
    // Process events loop
    while ((pollResult = ALooper_pollAll(0, NULL, &pollEvents, (void**)&CORE.Android.source)) >=
           0) {
      // Process this event
...
#if defined(PLATFORM_WEB)
  // Check fullscreen change events(note this is done on the window since most browsers don't
  // support this on #canvas)
  // emscripten_set_fullscreenchange_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL, 1,
  // EmscriptenResizeCallback);
  // Check Resize event (note this is done on the window since most browsers don't support this on
  // #canvas)
  ...
  if (CORE.Window.crtc) {
    if (CORE.Window.connector) {
      drmModeSetCrtc(CORE.Window.fd, CORE.Window.crtc->crtc_id, CORE.Window.crtc->buffer_id,
                     CORE.Window.crtc->x, CORE.Window.crtc->y, &CORE.Window.connector->connector_id,
                     1, &CORE.Window.crtc->mode);
...
void ToggleFullscreen(void) {
  ...
  if (!CORE.Window.fullscreen) {
    ...
    if (!monitor) {
      ...
      glfwSetWindowMonitor(CORE.Window.handle, NULL, 0, 0, CORE.Window.screen.width,
    ...
    glfwSetWindowMonitor(CORE.Window.handle, monitor, 0, 0, CORE.Window.screen.width,
                         CORE.Window.screen.height, GLFW_DONT_CARE);
  } else {
    CORE.Window.fullscreen = false; // Toggle fullscreen flag
    CORE.Window.flags &= ~FLAG_FULLSCREEN_MODE;
  ...
  /*
     ...
     //EMSCRIPTEN_FULLSCREEN_SCALE_ASPECT,
          ...
          TRACELOG(LOG_WARNING, "Emscripten: Enter fullscreen: Canvas size: %i x %i", width,
     height);
      }
      else
      {
...
void SetWindowState(unsigned int flags) {
  ...
  if (((CORE.Window.flags & FLAG_MSAA_4X_HINT) != (flags & FLAG_MSAA_4X_HINT)) &&
...
void SetWindowMonitor(int monitor) {
  ...
  if ((monitor >= 0) && (monitor < monitorCount)) {
    ...
    glfwSetWindowMonitor(CORE.Window.handle, monitors[monitor], 0, 0, mode->width, mode->height,
                         mode->refreshRate);
  } else
...
void BeginDrawing(void) {
  ...
  // consequently the measure for update vs draw is not accurate (only the total frame time is
  // accurate)
...
void EndDrawing(void) {
  ...
  if (!CORE.Input.Mouse.cursorHidden) {
    DrawRectangle(CORE.Input.Mouse.currentPosition.x, CORE.Input.Mouse.currentPosition.y, 3, 3,
                  MAROON);
    rlDrawRenderBatchActive(); // Update and draw internal render batch
  }
#endif
...
#define GIF_RECORD_FRAMERATE 10
    ...
    if ((gifFramesCounter % GIF_RECORD_FRAMERATE) == 0) {
      ...
      unsigned char* screenData =
          rlReadScreenPixels(CORE.Window.screen.width, CORE.Window.screen.height);
  ...
  if (eventsRecording)
    ...
    if (CORE.Time.frameCounter >= eventCount)
      eventsPlaying = false;
...
VrStereoConfig LoadVrStereoConfig(VrDeviceInfo device) {
  ...
  float lensShift =
      (device.hScreenSize * 0.25f - device.lensSeparationDistance * 0.5f) / device.hScreenSize;
  ...
  float distortionScale =
      device.lensDistortionValues[0] + device.lensDistortionValues[1] * lensRadiusSq +
  ...
  // some y (base of head to eye level) and -z (center of head to eye protrusion) to the camera
  // positions.
...
RLAPI Shader LoadShaderFromMemory(const char* vsCode, const char* fsCode) {
  ...
  if (shader.id > 0) {
    ...
    shader.locs[SHADER_LOC_VERTEX_POSITION] =
        rlGetLocationAttrib(shader.id, DEFAULT_SHADER_ATTRIB_NAME_POSITION);
    shader.locs[SHADER_LOC_VERTEX_TEXCOORD01] =
        rlGetLocationAttrib(shader.id, DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD);
    shader.locs[SHADER_LOC_VERTEX_TEXCOORD02] =
        rlGetLocationAttrib(shader.id, DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD2);
    shader.locs[SHADER_LOC_VERTEX_NORMAL] =
        rlGetLocationAttrib(shader.id, DEFAULT_SHADER_ATTRIB_NAME_NORMAL);
    shader.locs[SHADER_LOC_VERTEX_TANGENT] =
        rlGetLocationAttrib(shader.id, DEFAULT_SHADER_ATTRIB_NAME_TANGENT);
    shader.locs[SHADER_LOC_VERTEX_COLOR] =
        rlGetLocationAttrib(shader.id, DEFAULT_SHADER_ATTRIB_NAME_COLOR);

    // Get handles to GLSL uniform locations (vertex shader)
    shader.locs[SHADER_LOC_MATRIX_MVP] =
        rlGetLocationUniform(shader.id, DEFAULT_SHADER_UNIFORM_NAME_MVP);
    shader.locs[SHADER_LOC_MATRIX_VIEW] =
        rlGetLocationUniform(shader.id, DEFAULT_SHADER_UNIFORM_NAME_VIEW);
    shader.locs[SHADER_LOC_MATRIX_PROJECTION] =
        rlGetLocationUniform(shader.id, DEFAULT_SHADER_UNIFORM_NAME_PROJECTION);
    shader.locs[SHADER_LOC_MATRIX_MODEL] =
        rlGetLocationUniform(shader.id, DEFAULT_SHADER_UNIFORM_NAME_MODEL);
    shader.locs[SHADER_LOC_MATRIX_NORMAL] =
        rlGetLocationUniform(shader.id, DEFAULT_SHADER_UNIFORM_NAME_NORMAL);

    // Get handles to GLSL uniform locations (fragment shader)
    shader.locs[SHADER_LOC_COLOR_DIFFUSE] =
        rlGetLocationUniform(shader.id, DEFAULT_SHADER_UNIFORM_NAME_COLOR);
    shader.locs[SHADER_LOC_MAP_DIFFUSE] = rlGetLocationUniform(
        shader.id, DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE0); // SHADER_LOC_MAP_ALBEDO
    shader.locs[SHADER_LOC_MAP_SPECULAR] = rlGetLocationUniform(
        shader.id, DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE1); // SHADER_LOC_MAP_METALNESS
    shader.locs[SHADER_LOC_MAP_NORMAL] =
        rlGetLocationUniform(shader.id, DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE2);
  }
...
void SetShaderValueV(Shader shader, int locIndex, const void* value, int uniformType, int count) {
  ...
  // rlDisableShader();      // Avoid reseting current shader program, in case other uniforms are
  // set
}
...
Ray GetMouseRay(Vector2 mouse, Camera camera) {
  ...
  // We need this as the source position because orthographic projects, compared to perspect doesn't
  // have a convergence point, meaning that the "eye" of the camera is more like a plane than a
  // point.
...
Matrix GetCameraMatrix2D(Camera2D camera) {
  ...
  //      When setting higher scale, it's more intuitive for the world to become bigger (= camera
  //      become smaller), not for the camera getting bigger, hence the invert. Same deal with
  //      rotation.
  //   3. Move it by (-offset);
  //      Offset defines target transform relative to screen, but since we're effectively "moving"
  //      screen (camera) we need to do it into opposite direction (inverse transform)
  ...
  matTransform = MatrixMultiply(MatrixMultiply(matOrigin, MatrixMultiply(matScale, matRotation)),
                                matTranslation);
...
double GetTime(void) {
  ...
  unsigned long long int time =
      (unsigned long long int)ts.tv_sec * 1000000000LLU + (unsigned long long int)ts.tv_nsec;
...
void TakeScreenshot(const char* fileName) {
  ...
  Image image = {imgData, CORE.Window.render.width, CORE.Window.render.height, 1,
  ...
  emscripten_run_script(
      TextFormat("saveFileFromMEMFSToDisk('%s','%s')", GetFileName(path), GetFileName(path)));
#endif
...
const char* GetDirectoryPath(const char* filePath) {
  ...
  */
  ...
  // In case provided path does not contain a root drive letter (C:\, D:\) nor leading path
  // separator (\, /), we add the current directory path to dirPath
  ...
  if (lastSlash) {
    if (lastSlash == filePath) {
      ...
      memcpy(dirPath + (filePath[1] != ':' && filePath[0] != '\\' && filePath[0] != '/' ? 2 : 0),
             filePath, strlen(filePath) - (strlen(lastSlash) - 1));
...
#define MAX_DIRECTORY_FILES 512
  ...
  for (int i = 0; i < MAX_DIRECTORY_FILES; i++)
    dirFilesPath[i] = (char*)RL_MALLOC(MAX_FILEPATH_LENGTH * sizeof(char));
...
#define COMPRESSION_QUALITY_DEFLATE 8
  ...
  *compDataLength = sdeflate(&sdefl, compData, data, dataLength,
                             COMPRESSION_QUALITY_DEFLATE); // Compression level 8, same as stbwi

  TraceLog(LOG_INFO, "SYSTEM: Compress data: Original size: %i -> Comp. size: %i", dataLength,
           *compDataLength);
#endif
...
unsigned char* DecompressData(unsigned char* compData, int compDataLength, int* dataLength) {
  ...
  *dataLength = length;
  ...
  TraceLog(LOG_INFO, "SYSTEM: Decompress data: Comp. size: %i -> Original size: %i", compDataLength,
           *dataLength);
#endif
  ...
  if (fileData != NULL) {
    if (dataSize <= (position * sizeof(int))) {
      ...
      if (newFileData != NULL) {
        ...
        // RL_REALLOC failed
        TRACELOG(LOG_WARNING,
                 "FILEIO: [%s] Failed to realloc data (%u), position in bytes (%u) bigger than "
                 "actual file size",
                 path, dataSize, position * sizeof(int));
...
#else
  return (int)((CORE.Input.Mouse.currentPosition.x + CORE.Input.Mouse.offset.x) *
...
#else
  return (int)((CORE.Input.Mouse.currentPosition.y + CORE.Input.Mouse.offset.y) *
...
#else
  position.x =
      (CORE.Input.Mouse.currentPosition.x + CORE.Input.Mouse.offset.x) * CORE.Input.Mouse.scale.x;
  position.y =
      (CORE.Input.Mouse.currentPosition.y + CORE.Input.Mouse.offset.y) * CORE.Input.Mouse.scale.y;
#endif
...
void SetMousePosition(int x, int y) {
  ...
  glfwSetCursorPos(CORE.Window.handle, CORE.Input.Mouse.currentPosition.x,
...
Vector2 GetTouchPosition(int index) {
  ...
  if (index < MAX_TOUCH_POINTS)
    ...
    TRACELOG(LOG_WARNING, "INPUT: Required touch point out of range (Max touch points: %i)",
             MAX_TOUCH_POINTS);
  ...
  if ((CORE.Window.screen.width > CORE.Window.display.width) ||
    ...
    position.x = position.x * ((float)CORE.Window.render.width / (float)CORE.Window.display.width) -
                 CORE.Window.renderOffset.x / 2;
    position.y =
        position.y * ((float)CORE.Window.render.height / (float)CORE.Window.display.height) -
        CORE.Window.renderOffset.y / 2;
  }
#endif
...
static bool InitGraphicsDevice(int width, int height) {
  ...
  // NOTE: Framebuffer (render area - CORE.Window.render.width, CORE.Window.render.height) could
  // include black bars...
  ...
  // glfwWindowHint(GLFW_CLIENT_API, GLFW_OPENGL_API); // OpenGL API to use. Alternative:
  // GLFW_OPENGL_ES_API glfwWindowHint(GLFW_AUX_BUFFERS, 0);          // Number of auxiliar buffers
  ...
  if ((CORE.Window.flags & FLAG_WINDOW_HIGHDPI) > 0) {
    ...
    // NOTE: This hint only has an effect on platforms where screen coordinates and pixels always
    // map 1:1 such as Windows and X11. On platforms like macOS the resolution of the framebuffer is
    // changed independently of the window size.
    glfwWindowHint(GLFW_SCALE_TO_MONITOR, GLFW_TRUE); // Scale content area based on the monitor
  ...
  if (rlGetVersion() == OPENGL_21) {
    ...
    glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT,
                   GLFW_FALSE); // Fordward Compatibility Hint: Only 3.3 and above!
#endif
...
#if defined(PLATFORM_DESKTOP)
  // NOTE: GLFW 3.4+ defers initialization of the Joystick subsystem on the first call to any
  // Joystick related functions. Forcing this initialization here avoids doing it on
  // `PollInputEvents` called by `EndDrawing` after first frame has been just drawn. The
  // initialization will still happen and possible delays still occur, but before the window is
  // shown, which is a nicer experience. REF: https://github.com/raysan5/raylib/issues/1554
  if (MAX_GAMEPADS > 0)
  ...
  if (CORE.Window.fullscreen) {
    ...
    // Obtain recommended CORE.Window.display.width/CORE.Window.display.height from a valid
    // videomode for the monitor
    int count = 0;
    ...
    // Try to setup the most appropiate fullscreen framebuffer for the requested
    // screenWidth/screenHeight It considers device display resolution mode and setups a framebuffer
    // with black bars if required (render size/offset) Modified global variables:
    // CORE.Window.screen.width/CORE.Window.screen.height -
    // CORE.Window.render.width/CORE.Window.render.height -
    // CORE.Window.renderOffset.x/CORE.Window.renderOffset.y - CORE.Window.screenScale
    // TODO: It is a quite cumbersome solution to display size vs requested size, it should be
    // reviewed or removed... HighDPI monitors are properly considered in a following similar
    // function: SetupViewport()
    SetupFramebuffer(CORE.Window.display.width, CORE.Window.display.height);

    CORE.Window.handle = glfwCreateWindow(CORE.Window.display.width, CORE.Window.display.height,
                                          (CORE.Window.title != 0) ? CORE.Window.title : " ",
                                          glfwGetPrimaryMonitor(), NULL);

    // NOTE: Full-screen change, not working properly...
    // glfwSetWindowMonitor(CORE.Window.handle, glfwGetPrimaryMonitor(), 0, 0,
    // CORE.Window.screen.width, CORE.Window.screen.height, GLFW_DONT_CARE);
  } else {
  ...
  if (!CORE.Window.handle) {
    ...
    TRACELOG(LOG_INFO, "    > Viewport offsets: %i, %i", CORE.Window.renderOffset.x,
             CORE.Window.renderOffset.y);
  }

  // Set window callback events
  glfwSetWindowSizeCallback(CORE.Window.handle,
  ...
  glfwSetCursorPosCallback(CORE.Window.handle,
                           MouseCursorPosCallback); // Track mouse position changes
  ...
  if ((CORE.Window.screen.width <= 0) || (CORE.Window.screen.height <= 0)) {
    ...
    CORE.Window.modeIndex =
        FindMatchingConnectorMode(CORE.Window.connector, &CORE.Window.crtc->mode);
  ...
  if (CORE.Window.modeIndex < 0)
    CORE.Window.modeIndex =
        FindNearestConnectorMode(CORE.Window.connector, CORE.Window.screen.width,
                                 CORE.Window.screen.height, fps, allowInterlaced);
  ...
  if (CORE.Window.modeIndex < 0)
    CORE.Window.modeIndex = FindNearestConnectorMode(
        CORE.Window.connector, CORE.Window.screen.width, CORE.Window.screen.height, fps, true);
  ...
  TRACELOG(LOG_INFO, "DISPLAY: Selected DRM connector mode %s (%ux%u%c@%u)",
           ...
           (CORE.Window.connector->modes[CORE.Window.modeIndex].flags & DRM_MODE_FLAG_INTERLACE)
               ? 'i'
               : 'p',
  ...
  CORE.Window.gbmSurface = gbm_surface_create(
      CORE.Window.gbmDevice, CORE.Window.connector->modes[CORE.Window.modeIndex].hdisplay,
...
#endif
    // EGL_TRANSPARENT_TYPE, EGL_NONE, // Request transparent framebuffer (EGL_TRANSPARENT_RGB does
    // not work on RPI)
    EGL_DEPTH_SIZE,
  ...
  if (!eglChooseConfig(CORE.Window.device, framebufferAttribs, configs, numConfigs,
                       &matchingNumConfigs)) {
  ...
  CORE.Window.context =
      eglCreateContext(CORE.Window.device, CORE.Window.config, EGL_NO_CONTEXT, contextAttribs);
  if (CORE.Window.context == EGL_NO_CONTEXT) {
  ...
  // ANativeWindow_setBuffersGeometry() As soon as we picked a EGLConfig, we can safely reconfigure
  ...
  ANativeWindow_setBuffersGeometry(CORE.Android.app->window, CORE.Window.render.width,
                                   CORE.Window.render.height, displayFormat);
  // ANativeWindow_setBuffersGeometry(CORE.Android.app->window, 0, 0, displayFormat);       // Force
  // use of native display size

  CORE.Window.surface = eglCreateWindowSurface(CORE.Window.device, CORE.Window.config,
                                               CORE.Android.app->window, NULL);
  ...
  // NOTE: RPI dispmanx windowing system takes care of source rectangle scaling to destination
  // rectangle by hardware (no cost) Take care that renderWidth/renderHeight fit on
  ...
  alpha.opacity =
      255; // Set transparency level for framebuffer, requires EGLAttrib: EGL_TRANSPARENT_TYPE
  alpha.mask = 0;

  dispmanDisplay = vc_dispmanx_display_open(0); // LCD
  dispmanUpdate = vc_dispmanx_update_start(0);

  dispmanElement = vc_dispmanx_element_add(dispmanUpdate, dispmanDisplay, 0 /*layer*/, &dstRect,
                                           0 /*src*/, &srcRect, DISPMANX_PROTECTION_NONE, &alpha,
                                           0 /*clamp*/, DISPMANX_NO_ROTATE);
  ...
  CORE.Window.surface =
      eglCreateWindowSurface(CORE.Window.device, CORE.Window.config, &CORE.Window.handle, NULL);
...
#if defined(PLATFORM_DRM)
  CORE.Window.surface = eglCreateWindowSurface(CORE.Window.device, CORE.Window.config,
                                               (EGLNativeWindowType)CORE.Window.gbmSurface, NULL);
  ...
  if (eglMakeCurrent(CORE.Window.device, CORE.Window.surface, CORE.Window.surface,
                     CORE.Window.context) == EGL_FALSE) {
  ...
  // NOTE: CORE.Window.screen.width and CORE.Window.screen.height not used, just stored as globals
  // in rlgl
...
#if defined(PLATFORM_DESKTOP)
  if ((CORE.Window.flags & FLAG_WINDOW_HIGHDPI) > 0) {
    // NOTE: On APPLE platforms system should manage window/input scaling and also framebuffer
    // scaling Framebuffer scaling should be activated with:
    // glfwWindowHint(GLFW_COCOA_RETINA_FRAMEBUFFER, GLFW_TRUE);
#if !defined(__APPLE__)
    glfwGetFramebufferSize(CORE.Window.handle, &fbWidth, &fbHeight);

    // Screen scaling matrix is required in case desired screen area is different than display area
    CORE.Window.screenScale = MatrixScale((float)fbWidth / CORE.Window.screen.width,
...
static void SetupViewport(int width, int height) {
  ...
  // render area does not match full display area (this situation is only applicable on fullscreen
  // mode)
#if defined(__APPLE__)
  float xScale = 1.0f, yScale = 1.0f;
  glfwGetWindowContentScale(CORE.Window.handle, &xScale, &yScale);
  rlViewport(CORE.Window.renderOffset.x / 2 * xScale, CORE.Window.renderOffset.y / 2 * yScale,
             (CORE.Window.render.width - CORE.Window.renderOffset.x) * xScale,
             (CORE.Window.render.height - CORE.Window.renderOffset.y) * yScale);
#else
...
static void SetupFramebuffer(int width, int height) {
  // Calculate CORE.Window.render.width and CORE.Window.render.height, we have the display size
  // (input params) and the desired screen size (global var)
  if ((CORE.Window.screen.width > CORE.Window.display.width) ||
      (CORE.Window.screen.height > CORE.Window.display.height)) {
    TRACELOG(
        LOG_WARNING,
...
// tasks more often. High resolutions can also prevent the CPU power management system from entering
// power-saving modes. Setting a higher resolution does not improve the accuracy of the
// high-resolution performance counter.
  ...
  {
    CORE.Time.base =
        (unsigned long long int)now.tv_sec * 1000000000LLU + (unsigned long long int)now.tv_nsec;
  } else
...
void WaitTime(float ms) {
  ...
  // Busy wait loop
  while ((currentTime - previousTime) < ms / 1000.0f)
    currentTime = GetTime();
#else
...
// System halt functions
  ...
  while ((currentTime - previousTime) < busyWait / 1000.0f)
    currentTime = GetTime();
#endif
#endif
}
...
void SwapScreenBuffer(void) {
...
#if defined(PLATFORM_DRM)
  if (!CORE.Window.gbmSurface || (-1 == CORE.Window.fd) || !CORE.Window.connector ||
      !CORE.Window.crtc) {
  ...
  uint32_t fb = 0;
  int result =
      drmModeAddFB(CORE.Window.fd, CORE.Window.connector->modes[CORE.Window.modeIndex].hdisplay,
                   CORE.Window.connector->modes[CORE.Window.modeIndex].vdisplay, 24, 32,
  ...
  result = drmModeSetCrtc(CORE.Window.fd, CORE.Window.crtc->crtc_id, fb, 0, 0,
                          &CORE.Window.connector->connector_id, 1,
...
void PollInputEvents(void) {
  ...
  for (int i = 0; i < MAX_GAMEPADS; i++) {
    if (CORE.Input.Gamepad.ready[i]) // Check if gamepad is available
    {
      ...
      glfwGetGamepadState(i, &state); // This remapps all gamepads so they have their buttons mapped
      ...
      for (int k = 0; (axes != NULL) && (k < GLFW_GAMEPAD_AXIS_LAST + 1) && (k < MAX_GAMEPAD_AXIS);
           k++) {
        CORE.Input.Gamepad.axisState[i][k] = axes[k];
      }

      // Register buttons for 2nd triggers (because GLFW doesn't count these as buttons but rather
      // axis)
...
// NOTE: GLFW3 joystick functionality not available in web
  ...
  // NOTE: Keyboard reading could be done using input_event(s) or just read from stdin, both methods
  // are used here. stdin reading is still used for legacy purposes, it allows keyboard input trough
  // SSH console

  if (!CORE.Input.Keyboard.evtMode)
    ProcessKeyboard();

    // NOTE: Mouse input events polling is done asynchronously in another pthread - EventThread()
    // NOTE: Gamepad (Joystick) input events polling is done asynchonously in another pthread -
    // GamepadThread()
#endif
}
...
static void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
  // TRACELOG(LOG_DEBUG, "Key Callback: KEY:%i(%c) - SCANCODE:%i (STATE:%i)", key, key, scancode,
  // action);
  ...
  if (key == CORE.Input.Keyboard.exitKey && action == GLFW_PRESS) {
    ...
    if (mods == GLFW_MOD_CONTROL) {
      if (gifRecording) {
        ...
        SaveFileData(TextFormat("%s/screenrec%03i.gif", CORE.Storage.basePath, screenshotCounter),
    ...
    if (!eventsRecording)
      ExportAutomationEvents("eventsrec.rep");
...
static void AndroidCommandCallback(struct android_app* app, int32_t cmd) {
  ...
  case APP_CMD_INIT_WINDOW: {
    if (app->window != NULL) {
      if (CORE.Android.contextRebindRequired) {
        ...
        EGLint displayFormat = 0;
        eglGetConfigAttrib(CORE.Window.device, CORE.Window.config, EGL_NATIVE_VISUAL_ID,
                           &displayFormat);
        ANativeWindow_setBuffersGeometry(app->window, CORE.Window.render.width,
                                         CORE.Window.render.height, displayFormat);

        // Recreate display surface and re-attach OpenGL context
        CORE.Window.surface =
            eglCreateWindowSurface(CORE.Window.device, CORE.Window.config, app->window, NULL);
        eglMakeCurrent(CORE.Window.device, CORE.Window.surface, CORE.Window.surface,
                       CORE.Window.context);
  ...
  case APP_CMD_TERM_WINDOW: {
    ...
    // NOTE 1: Detaching context before destroying display surface avoids losing our resources
    // (textures, shaders, VBOs...) NOTE 2: In some cases (too many context loaded), OS could unload
  ...
  case APP_CMD_CONFIG_CHANGED: {
    // AConfiguration_fromAssetManager(CORE.Android.app->config,
    // CORE.Android.app->activity->assetManager); print_cur_config(CORE.Android.app);
...
static int32_t AndroidInputCallback(struct android_app* app, AInputEvent* event) {
  ...
  if (type == AINPUT_EVENT_TYPE_MOTION) {
    ...
    if (keycode == AKEYCODE_POWER) {
      // Let the OS handle input to avoid app stuck. Behaviour: CMD_PAUSE -> CMD_SAVE_STATE ->
      // CMD_STOP -> CMD_CONFIG_CHANGED -> CMD_LOST_FOCUS Resuming Behaviour: CMD_START ->
      // CMD_RESUME -> CMD_CONFIG_CHANGED -> CMD_CONFIG_CHANGED -> CMD_GAINED_FOCUS It seems like
      // locking mobile, screen size (CMD_CONFIG_CHANGED) is affected. NOTE: AndroidManifest.xml
      // must have <activity android:configChanges="orientation|keyboardHidden|screenSize" > Before
      // that change, activity was calling CMD_TERM_WINDOW and CMD_DESTROY when locking mobile, so
      // that was not a normal behaviour
      return 0;
...
static EM_BOOL EmscriptenTouchCallback(int eventType, const EmscriptenTouchEvent* touchEvent,
  ...
  // Register touch points position
  // NOTE: Only two points registered
  gestureEvent.position[0] =
      (Vector2){touchEvent->touches[0].targetX, touchEvent->touches[0].targetY};
  gestureEvent.position[1] =
      (Vector2){touchEvent->touches[1].targetX, touchEvent->touches[1].targetY};

  double canvasWidth, canvasHeight;
  // NOTE: emscripten_get_canvas_element_size() returns canvas.width and canvas.height but
  // we are looking for actual CSS size: canvas.style.width and canvas.style.height
  // EMSCRIPTEN_RESULT res = emscripten_get_canvas_element_size("#canvas", &canvasWidth,
  // &canvasHeight);
...
static EM_BOOL EmscriptenGamepadCallback(int eventType, const EmscriptenGamepadEvent* gamepadEvent,
                                         void* userData) {
  /*
  TRACELOGD("%s: timeStamp: %g, connected: %d, index: %ld, numAxes: %d, numButtons: %d, id: \"%s\",
  mapping: \"%s\"", eventType != 0? emscripten_event_type_to_string(eventType) : "Gamepad state",
         gamepadEvent->timestamp, gamepadEvent->connected, gamepadEvent->index,
  gamepadEvent->numAxes, gamepadEvent->numButtons, gamepadEvent->id, gamepadEvent->mapping);

  for (int i = 0; i < gamepadEvent->numAxes; ++i) TRACELOGD("Axis %d: %g", i,
  gamepadEvent->axis[i]); for (int i = 0; i < gamepadEvent->numButtons; ++i) TRACELOGD("Button %d:
  Digital: %d, Analog: %g", i, gamepadEvent->digitalButton[i], gamepadEvent->analogButton[i]);
  */
...
static void InitKeyboard(void) {
  ...
  // New terminal settings for keyboard: turn off buffering (non-canonical mode), echo and key
  // processing NOTE: ISIG controls if ^C and ^Z generate break signals or not
  ...
  CORE.Input.Keyboard.defaultFileFlags = fcntl(
      STDIN_FILENO, F_GETFL, 0); // F_GETFL: Get the file access mode and the file status flags
  fcntl(STDIN_FILENO, F_SETFL,
        CORE.Input.Keyboard.defaultFileFlags |
            O_NONBLOCK); // F_SETFL: Set the file status flags to the value specified
...
// Initialise user input from evdev(/dev/input/event<N>) this means mouse, keyboard or gamepad
// devices
...
#define TEST_BIT(array, bit) ((array[LONG(bit)] >> OFF(bit)) & 1)
  ...
  if (freeWorkerId >= 0) {
    ...
    TRACELOG(LOG_WARNING, "RPI: Failed to create input device thread for %s, out of worker slots",
             device);
    return;
  }
  ...
  // Identify the device
  //-------------------------------------------------------------------------------------------------------
  ioctl(fd, EVIOCGBIT(0, sizeof(evBits)),
        evBits); // Read a bitfield of the available device properties
  ...
  if (worker->isKeyboard && CORE.Input.Keyboard.fd == -1) {
    // Use the first keyboard encountered. This assumes that a device that says it's a keyboard is
    // just a keyboard. The keyboard is polled synchronously, whereas other input devices are polled
    // in separate threads so that they don't drop events when the frame rate is slow.
    ...
    TRACELOG(LOG_INFO, "RPI: Opening input device: %s (%s%s%s%s)", device,
             worker->isMouse ? "mouse " : "", worker->isMultitouch ? "multitouch " : "",
             worker->isTouch ? "touchscreen " : "", worker->isGamepad ? "gamepad " : "");

    // Create a thread for this device
    int error = pthread_create(&worker->threadId, NULL, &EventThread, (void*)worker);
    if (error != 0) {
      TRACELOG(LOG_WARNING, "RPI: Failed to create input device thread: %s (error: %d)", device,
               error);
      worker->threadId = 0;
      close(fd);
    }
    ...
    for (int i = 0; i < sizeof(CORE.Input.eventWorker) / sizeof(InputEventWorker); ++i) {
      if (CORE.Input.eventWorker[i].isTouch &&
          (CORE.Input.eventWorker[i].eventNum > maxTouchNumber))
...
static void PollKeyboardEvents(void) {
  ...
  // TODO: Probably replace this with a keymap from the X11 to get the correct regional map for the
  // keyboard: Currently non US keyboards will have the wrong mapping for some keys
  static const int keymapUS[] = {
      0,   256, 49,  50,  51,  52,  53,  54,  55,  56,  57,  48,  45,  61,  259, 258, 81,  87,  69,
      82,  84,  89,  85,  73,  79,  80,  91,  93,  257, 341, 65,  83,  68,  70,  71,  72,  74,  75,
      76,  59,  39,  96,  340, 92,  90,  88,  67,  86,  66,  78,  77,  44,  46,  47,  344, 332, 342,
      32,  280, 290, 291, 292, 293, 294, 295, 296, 297, 298, 299, 282, 281, 327, 328, 329, 333, 324,
      325, 326, 334, 321, 322, 323, 320, 330, 0,   85,  86,  300, 301, 89,  90,  91,  92,  93,  94,
      95,  335, 345, 331, 283, 346, 101, 268, 265, 266, 263, 262, 269, 264, 267, 260, 261, 112, 113,
      114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 124, 125, 347, 127, 128, 129, 130, 131, 132,
      133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151,
      152, 153, 154, 155, 156, 157, 158, 159, 160, 161, 162, 163, 164, 165, 166, 167, 168, 169, 170,
      171, 172, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189,
      190, 191, 192, 193, 194, 0,   0,   0,   0,   0,   200, 201, 202, 203, 204, 205, 206, 207, 208,
      209, 210, 211, 212, 213, 214, 215, 216, 217, 218, 219, 220, 221, 222, 223, 224, 225, 226, 227,
      228, 229, 230, 231, 232, 233, 234, 235, 236, 237, 238, 239, 240, 241, 242, 243, 244, 245, 246,
      247, 248, 0,   0,   0,   0,   0,   0,   0};
    ...
    if (event.type == EV_KEY) {
      ...
      if ((event.code >= 1) && (event.code <= 255)) // Keyboard keys appear for codes 1 to 255
      {
        ...
        if ((keycode > 0) && (keycode < sizeof(CORE.Input.Keyboard.currentKeyState))) {
          ...
          // Event interface: 'value' is the value the event carries. Either a relative change for
          // EV_REL, absolute new value for EV_ABS (joysticks ...), or 0 for EV_KEY for release, 1
  ...
  while (!CORE.Window.shouldClose) {
    ...
    while (read(worker->fd, &event, sizeof(event)) == (int)sizeof(event)) {
      ...
      if (event.type == EV_ABS) {
        // Basic movement
        if (event.code == ABS_X) {
          CORE.Input.Mouse.currentPosition.x = (event.value - worker->absRange.x) *
                                               CORE.Window.screen.width /
                                               worker->absRange.width; // Scale acording to absRange
          CORE.Input.Touch.position[0].x = (event.value - worker->absRange.x) *
                                           CORE.Window.screen.width /
                                           worker->absRange.width; // Scale acording to absRange
        ...
        if (event.code == ABS_Y) {
          CORE.Input.Mouse.currentPosition.y =
              (event.value - worker->absRange.y) * CORE.Window.screen.height /
              worker->absRange.height; // Scale acording to absRange
          CORE.Input.Touch.position[0].y = (event.value - worker->absRange.y) *
                                           CORE.Window.screen.height /
                                           worker->absRange.height; // Scale acording to absRange
        ...
        if (event.code == ABS_MT_POSITION_X) {
          if (worker->touchSlot < MAX_TOUCH_POINTS)
            CORE.Input.Touch.position[worker->touchSlot].x =
                (event.value - worker->absRange.x) * CORE.Window.screen.width /
                worker->absRange.width; // Scale acording to absRange
        }

        if (event.code == ABS_MT_POSITION_Y) {
          if (worker->touchSlot < MAX_TOUCH_POINTS)
            CORE.Input.Touch.position[worker->touchSlot].y =
                (event.value - worker->absRange.y) * CORE.Window.screen.height /
                worker->absRange.height; // Scale acording to absRange
        }
        ...
        if (event.code == ABS_PRESSURE) {
          int previousMouseLeftButtonState =
              CORE.Input.Mouse.currentButtonStateEvdev[MOUSE_BUTTON_LEFT];
      ...
      if (event.type == EV_KEY) {
        ...
        if (event.code == BTN_RIGHT)
          CORE.Input.Mouse.currentButtonStateEvdev[MOUSE_BUTTON_RIGHT] = event.value;
        if (event.code == BTN_MIDDLE)
          CORE.Input.Mouse.currentButtonStateEvdev[MOUSE_BUTTON_MIDDLE] = event.value;
        if (event.code == BTN_SIDE)
          CORE.Input.Mouse.currentButtonStateEvdev[MOUSE_BUTTON_SIDE] = event.value;
        if (event.code == BTN_EXTRA)
          CORE.Input.Mouse.currentButtonStateEvdev[MOUSE_BUTTON_EXTRA] = event.value;
        if (event.code == BTN_FORWARD)
          CORE.Input.Mouse.currentButtonStateEvdev[MOUSE_BUTTON_FORWARD] = event.value;
        if (event.code == BTN_BACK)
          CORE.Input.Mouse.currentButtonStateEvdev[MOUSE_BUTTON_BACK] = event.value;
      }
    ...
    for (int i = 0; i < MAX_GAMEPADS; i++) {
      if (read(CORE.Input.Gamepad.streamId[i], &gamepadEvent, sizeof(struct js_event)) ==
        ...
        if (gamepadEvent.type == JS_EVENT_BUTTON) {
          // TRACELOG(LOG_WARNING, "RPI: Gamepad button: %i, value: %i", gamepadEvent.number,
          // gamepadEvent.value);
          ...
          // TRACELOG(LOG_WARNING, "RPI: Gamepad axis: %i, value: %i", gamepadEvent.number,
          // gamepadEvent.value);
...
static int FindExactConnectorMode(const drmModeConnector* connector, uint width, uint height,
                                  uint fps, bool allowInterlaced) {
  TRACELOG(LOG_TRACE,
           "DISPLAY: Searching exact connector mode for %ux%u@%u, selecting an interlaced mode is "
           "allowed: %s",
  ...
  for (int i = 0; i < CORE.Window.connector->count_modes; i++) {
    ...
    TRACELOG(LOG_TRACE, "DISPLAY: DRM Mode %d %ux%u@%u %s", i, mode->hdisplay, mode->vdisplay,
             mode->vrefresh,
...
static int FindNearestConnectorMode(const drmModeConnector* connector, uint width, uint height,
                                    uint fps, bool allowInterlaced) {
  TRACELOG(LOG_TRACE,
           "DISPLAY: Searching nearest connector mode for %ux%u@%u, selecting an interlaced mode "
           "is allowed: %s",
           width, height, fps, allowInterlaced ? "yes" : "no");
  ...
  if (repFile != NULL) {
    ...
    for (int i = 0; i < eventCount; i++) {
      fprintf(repFile, "e %i %i %i %i %i // %s\n", events[i].frame, events[i].type,
              events[i].params[0], events[i].params[1], events[i].params[2],
              autoEventTypeName[events[i].type]);
    }
...
static void RecordAutomationEvent(unsigned int frame) {
  for (int key = 0; key < MAX_KEYBOARD_KEYS; key++) {
    ...
    if (CORE.Input.Keyboard.previousKeyState[key] && !CORE.Input.Keyboard.currentKeyState[key]) {
      ...
      TRACELOG(LOG_INFO, "[%i] INPUT_KEY_UP: %i, %i, %i", events[eventCount].frame,
               events[eventCount].params[0], events[eventCount].params[1],
               events[eventCount].params[2]);
      eventCount++;
    }
    ...
    if (CORE.Input.Keyboard.currentKeyState[key]) {
      ...
      TRACELOG(LOG_INFO, "[%i] INPUT_KEY_DOWN: %i, %i, %i", events[eventCount].frame,
               events[eventCount].params[0], events[eventCount].params[1],
               events[eventCount].params[2]);
      eventCount++;
    }
  ...
  if (((int)CORE.Input.Mouse.currentPosition.x != (int)CORE.Input.Mouse.previousPosition.x) ||
    ...
    TRACELOG(LOG_INFO, "[%i] INPUT_MOUSE_POSITION: %i, %i, %i", events[eventCount].frame,
             events[eventCount].params[0], events[eventCount].params[1],
             events[eventCount].params[2]);
    eventCount++;
  }
  ...
  for (int id = 0; id < MAX_TOUCH_POINTS; id++) {
    ...
    if (CORE.Input.Touch.previousTouchState[id] && !CORE.Input.Touch.currentTouchState[id]) {
      ...
      TRACELOG(LOG_INFO, "[%i] INPUT_TOUCH_UP: %i, %i, %i", events[eventCount].frame,
               events[eventCount].params[0], events[eventCount].params[1],
               events[eventCount].params[2]);
      eventCount++;
    }
    ...
    if (CORE.Input.Touch.currentTouchState[id]) {
      ...
      TRACELOG(LOG_INFO, "[%i] INPUT_TOUCH_DOWN: %i, %i, %i", events[eventCount].frame,
               events[eventCount].params[0], events[eventCount].params[1],
               events[eventCount].params[2]);
      eventCount++;
    }
  ...
  if (GESTURES.current != GESTURE_NONE) {
    ...
    TRACELOG(LOG_INFO, "[%i] INPUT_GESTURE: %i, %i, %i", events[eventCount].frame,
             events[eventCount].params[0], events[eventCount].params[1],
             events[eventCount].params[2]);
    eventCount++;
  }
  ...
  for (unsigned int i = 0; i < eventCount; i++) {
    if (events[i].frame == frame) {
      ...
      case INPUT_GESTURE:
        ...
        break; // param[0]: gesture (enum Gesture) -> gestures.h: GESTURES.current
