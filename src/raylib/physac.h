 ...
 *   DESCRIPTION:
 *
 *   Physac is a small 2D physics engine written in pure C. The engine uses a fixed time-step thread
 *loop to simluate physics. A physics step contains the following phases: get collision information,
 *   apply dynamics, collision solving and position correction. It uses a very simple struct for
 *physic bodies with a position vector to be used in any 3D rendering API.
 *
 *   CONFIGURATION:
 *
 ...
 *
 *   #define PHYSAC_DEBUG
 *       Show debug traces log messages about physic bodies creation/destruction, physic system
 *errors, some calculations results and NULL reference exceptions
 *
 ...
 *       it allows just running UpdatePhysics() automatically on a separate thread at a desired time
 *step. In case physics steps update needs to be controlled by user with a custom timming mechanism,
 ...
 *       gcc -o $(NAME_PART).exe $(FILE_NAME) -s -static -lraylib -lopengl32 -lgdi32 -lwinmm
 *-std=c99
 *
 *   VERSIONS HISTORY:
...
CreatePhysicsBodyCircle(Vector2 pos, float radius,
...
PHYSACDEF void PhysicsShatter(PhysicsBody body, Vector2 position,
                              float force); // Shatters a polygon shape physics body to little
                                            // physics bodies with explosion force
PHYSACDEF void SetPhysicsBodyRotation(
    PhysicsBody body,
    float radians); // Sets physics body shape transform based on radians parameter

// Query physics info
PHYSACDEF PhysicsBody
GetPhysicsBody(int index); // Returns a physics body of the bodies pool at a specific index
...
PHYSACDEF int
GetPhysicsShapeVerticesCount(int index); // Returns the amount of vertices of a physics body shape
...
static double deltaTime =
    1.0 / 60.0 / 10.0 * 1000; // Delta time in milliseconds used for physics steps
...
static void
UpdatePhysicsStep(void); // Update physics step (dynamics, collisions and position corrections)
...
CreatePhysicsManifold(PhysicsBody a,
                      PhysicsBody b); // Creates a new physics manifold to solve collision
static void
DestroyPhysicsManifold(PhysicsManifold manifold); // Unitializes and destroys a physics manifold
...
static void SolveCircleToPolygon(PhysicsManifold manifold); // Solves collision between a circle to
                                                            // a polygon shape physics bodies
static void SolvePolygonToCircle(PhysicsManifold manifold); // Solves collision between a polygon to
                                                            // a circle shape physics bodies
...
static void
IntegratePhysicsVelocity(PhysicsBody body); // Integrates physics velocity into position and forces
static void IntegratePhysicsImpulses(
    PhysicsManifold manifold); // Integrates physics collisions impulses to solve collisions
static void
CorrectPhysicsPositions(PhysicsManifold manifold); // Corrects physics bodies positions based on
...
static float
FindAxisLeastPenetration(int* faceIndex, PhysicsShape shapeA,
                         PhysicsShape shapeB); // Finds polygon shapes axis least penetration

// Math required functions
static Vector2 MathVector2Product(Vector2 vector,
                                  float value); // Returns the product of a vector and a value
static float MathVector2CrossProduct(Vector2 v1,
...
static float MathVector2DotProduct(Vector2 v1,
                                   Vector2 v2); // Returns the dot product of two vectors
static inline float
MathVector2SqrDistance(Vector2 v1,
...
static Vector2 MathVector2Subtract(Vector2 v1,
                                   Vector2 v2); // Returns the subtract of two given vectors
static Matrix2x2
MathMatFromRadians(float radians); // Returns a matrix 2x2 from a given radians value
static inline Matrix2x2
MathMatTranspose(Matrix2x2 matrix); // Returns the transpose of a given matrix 2x2
static inline Vector2
MathMatVector2Product(Matrix2x2 matrix,
...
static Vector2
MathTriangleBarycenter(Vector2 v1, Vector2 v2,
  ...
  if (body != NULL) {
    if (body->shape.type == PHYSICS_POLYGON) {
      ...
      for (unsigned int i = 0; i < vertexData.vertexCount; i++) {
        ...
        Vector2 positionB = MathMatVector2Product(
            body->shape.transform, MathVector2Add(body->position, vertexData.positions[i]));
        unsigned int nextIndex = (((i + 1) < vertexData.vertexCount) ? (i + 1) : 0);
        Vector2 positionC = MathMatVector2Product(
            body->shape.transform, MathVector2Add(body->position, vertexData.positions[nextIndex]));
      ...
      if (collision) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          Vector2 center =
              MathTriangleBarycenter(vertices[i], vertices[nextIndex], PHYSAC_VECTOR_ZERO);
          ...
          for (unsigned int j = 0; j < vertexData.vertexCount; j++) {
            ...
            Vector2 face =
                MathVector2Subtract(vertexData.positions[nextVertex], vertexData.positions[j]);
          ...
          for (unsigned int j = 0; j < body->shape.vertexData.vertexCount; j++) {
            ...
            unsigned int nextVertex =
                (((j + 1) < body->shape.vertexData.vertexCount) ? (j + 1) : 0);
          ...
          Vector2 forceDirection = MathVector2Subtract(
              MathVector2Add(pointA, MathVector2Add(vertexData.positions[0], pointB)),
              body->position);
  ...
  if (body != NULL) {
    ...
    case PHYSICS_CIRCLE: {
      position.x = body->position.x +
                   cosf(360.0f / PHYSAC_DEFAULT_CIRCLE_VERTICES * vertex * PHYSAC_DEG2RAD) *
                       body->shape.radius;
      position.y = body->position.y +
                   sinf(360.0f / PHYSAC_DEFAULT_CIRCLE_VERTICES * vertex * PHYSAC_DEG2RAD) *
                       body->shape.radius;
    } break;
    case PHYSICS_POLYGON: {
      PhysicsVertexData vertexData = body->shape.vertexData;
      position =
          MathVector2Add(body->position, MathMatVector2Product(body->shape.transform,
...
PHYSACDEF void ClosePhysics(void) {
  ...
  if ((physicsBodiesCount > 0) || (usedMemory != 0)) {
    TRACELOG("[PHYSAC] WARNING: Physics module closed with unallocated bodies (BODIES: %i, MEMORY: "
             "%i bytes)\n",
             physicsBodiesCount, usedMemory);
  } else if ((physicsManifoldsCount > 0) || (usedMemory != 0)) {
    TRACELOG("[PHYSAC] WARNING: Pysics module closed with unallocated manifolds (MANIFOLDS: %i, "
             "MEMORY: %i bytes)\n",
      ...
      for (unsigned int j = i + 1; j < physicsBodiesCount; j++) {
        ...
        if (bodyB != NULL) {
          ...
          if (manifold->contactsCount > 0) {
            // Create a new manifold with same information as previously solved manifold and add it
            // to the manifolds pool last slot
            PhysicsManifold manifold = CreatePhysicsManifold(bodyA, bodyB);
...
static void SolveCircleToCircle(PhysicsManifold manifold) {
  ...
  if (distance == 0.0f) {
    ...
    manifold->normal = CLITERAL(Vector2){
        normal.x / distance,
        normal.y /
            distance}; // Faster than using MathVector2Normalize() due to sqrt is already performed
    manifold->contacts[0] =
        CLITERAL(Vector2){manifold->normal.x * bodyA->shape.radius + bodyA->position.x,
...
static void SolveCircleToPolygon(PhysicsManifold manifold) {
  ...
  center = MathMatVector2Product(MathMatTranspose(bodyB->shape.transform),
                                 MathVector2Subtract(center, bodyB->position));
  ...
  for (unsigned int i = 0; i < vertexData.vertexCount; i++) {
    float currentSeparation = MathVector2DotProduct(
        vertexData.normals[i], MathVector2Subtract(center, vertexData.positions[i]));
...
static void SolvePolygonToPolygon(PhysicsManifold manifold) {
  ...
  // MathVector2Clip incident face to reference face side planes (due to floating point error,
  // possible to not have required points
  if (MathVector2Clip(CLITERAL(Vector2){-sidePlaneNormal.x, -sidePlaneNormal.y}, &incidentFace[0],
                      &incidentFace[1], negSide) < 2)
    return;
...
static void InitializePhysicsManifolds(PhysicsManifold manifold) {
  ...
  for (unsigned int i = 0; i < manifold->contactsCount; i++) {
    ...
    if (MathVector2SqrLen(radiusV) <
        (MathVector2SqrLen(CLITERAL(Vector2){(float)(gravityForce.x * deltaTime / 1000),
...
static void IntegratePhysicsImpulses(PhysicsManifold manifold) {
  ...
  for (unsigned int i = 0; i < manifold->contactsCount; i++) {
    ...
    Vector2 radiusV = {0.0f, 0.0f};
    radiusV.x = bodyB->velocity.x + MathVector2Product(radiusB, bodyB->angularVelocity).x -
                bodyA->velocity.x - MathVector2Product(radiusA, bodyA->angularVelocity).x;
    radiusV.y = bodyB->velocity.y + MathVector2Product(radiusB, bodyB->angularVelocity).y -
                bodyA->velocity.y - MathVector2Product(radiusA, bodyA->angularVelocity).y;
    ...
    if (bodyA->enabled) {
      ...
      if (!bodyA->freezeOrient)
        bodyA->angularVelocity +=
            ...
            MathVector2CrossProduct(radiusA, CLITERAL(Vector2){-impulseV.x, -impulseV.y});
    }
    ...
    if (bodyB->enabled) {
      ...
      if (!bodyB->freezeOrient)
        bodyB->angularVelocity +=
            bodyB->inverseInertia * MathVector2CrossProduct(radiusB, impulseV);
    }
    ...
    Vector2 tangent = {
        radiusV.x - (manifold->normal.x * MathVector2DotProduct(radiusV, manifold->normal)),
    ...
    if (bodyA->enabled) {
      ...
      if (!bodyA->freezeOrient)
        bodyA->angularVelocity +=
            bodyA->inverseInertia *
            MathVector2CrossProduct(radiusA,
                                    CLITERAL(Vector2){-tangentImpulse.x, -tangentImpulse.y});
    }
    ...
    if (bodyB->enabled) {
      ...
      if (!bodyB->freezeOrient)
        bodyB->angularVelocity +=
            bodyB->inverseInertia * MathVector2CrossProduct(radiusB, tangentImpulse);
    }
  }
}
...
static void FindIncidentFace(Vector2* v0, Vector2* v1, PhysicsShape ref, PhysicsShape inc,
  ...
  referenceNormal = MathMatVector2Product(MathMatTranspose(inc.transform),
...
static unsigned long long int GetClockTicks(void) {
  ...
  value = (unsigned long long int)now.tv_sec * (unsigned long long int)1000000000 +
...
static inline Vector2 MathMatVector2Product(Matrix2x2 matrix, Vector2 vector) {
  Vector2 result = {matrix.m00 * vector.x + matrix.m01 * vector.y,
