    ...
    if ptype == MSG_CONNECT:
        if payload == b'remote-mouse' and current_stream_id is None:
            if mouse_available:
                ...
                logger.warning('No host mouse present at %s; rejecting remote-mouse connect', DEV_NAME)
...
if not done:
    try:
        ...
        logger.warning('No mouse present at %s; remote-mouse will stay registered but reject connects', DEV_NAME)
...
while not done:
    ...
    if buffered_movement:
        now = time.time()
        if next_send <= now and current_stream_id is not None:
    ...
    for fd in rfd:
        ...
        elif dev_fd is not None and fd == dev_fd:
