...
int jit_block_emit(jit_context_t *jit, jit_block_t *block)
{
    ...
    for (int i = 0; i < block->instruction_count; i++) {
        ...
        switch (opinfo->family) {
            ...
            case JIT_FAMILY_BCHG:
                ...
                break;
            default:
                /* DBcc is frequently misclassified as SCC in generated opinfo. */
                if ((opcode & 0xF0F8) == 0x50C8) {
                    translator = jit_translate_misc;
                    break;
                }
        ...
        block->host_offsets[i] = (uint16_t)emit.offset;
    ...
    block->host_offsets[block->instruction_count] = (uint16_t)emit.offset;
...
int jit_block_execute(jit_block_t *block, int max_cycles)
{
    ...
    g_jit.current_block = block;
    int cycles = func(max_cycles);
    g_jit.current_block = NULL;
...
int jit_translate_move(jit_translate_context_t *tctx)
{
    ...
    if (dst_mode == 0) {
        ...
        if (src_mode == 0) {
            ...
            if (size == 0) {
                jit_emit_call_read8(ctx, AARCH64_R1, AARCH64_R0);
                jit_emit_store_dn(ctx, AARCH64_R1, dst_reg);
            } else if (size == 1) {
                jit_emit_call_read16(ctx, AARCH64_R1, AARCH64_R0);
                jit_emit_store_dn(ctx, AARCH64_R1, dst_reg);
            } else {
                jit_emit_call_read32(ctx, AARCH64_R1, AARCH64_R0);
                jit_emit_store_dn(ctx, AARCH64_R1, dst_reg);
            }
        } else if (src_mode == 3) {
            /* MOVE (An)+, Dn */
        ...
        if (src_mode == 0) {
            ...
            if (size == 0) {
                jit_emit_call_write8(ctx, AARCH64_R0, AARCH64_R1);
            } else if (size == 1) {
                jit_emit_call_write16(ctx, AARCH64_R0, AARCH64_R1);
            } else {
                jit_emit_call_write32(ctx, AARCH64_R0, AARCH64_R1);
            }
        } else {
            return -1;
        }
...
int jit_translate_add(jit_translate_context_t *tctx)
{
     ...
     *   001 ADD.W <ea>,Dn
     *   010 ADD.L <ea>,Dn
     *   100/101/110 ADD Dn,<ea> (not handled here yet)
     */
    uint8_t opmode = (opcode >> 6) & 0x7;
    uint8_t dst_reg = (opcode >> 9) & 0x7; /* Dn when opmode <= 2 */
    uint8_t src_mode = (opcode >> 3) & 0x7;
    uint8_t src_reg = opcode & 0x7;
    int incr;
    ...
    /* Start with destination Dn. */
    jit_emit_load_dn(ctx, AARCH64_R0, dst_reg);

    /* Load source operand into R1. */
    if (src_mode == 0) {
        /* ADD Dn,Dn */
        ...
        if (opmode == 0) {
            ...
            jit_emit_call_read16(ctx, AARCH64_R1, AARCH64_R2);
        } else {
            jit_emit_call_read32(ctx, AARCH64_R1, AARCH64_R2);
        }
    } else if (src_mode == 3) {
        /* ADD (An)+,Dn */
        jit_emit_load_an(ctx, AARCH64_R2, src_reg);
        ...
        jit_emit_load_an(ctx, AARCH64_R2, src_reg);
        jit_emit_mov64(ctx, AARCH64_R3, (uint64_t)incr);
        jit_emit_dword(ctx, AARCH64_ADD(AARCH64_R2, AARCH64_R2, AARCH64_R3));
        jit_emit_store_an(ctx, AARCH64_R2, src_reg);
    } else if (src_mode == 5) {
        /* ADD d16(An),Dn */
        int16_t disp = (int16_t)ext_words[0];
        jit_emit_load_an(ctx, AARCH64_R2, src_reg);
    ...
    if (opmode == 0) {
        jit_emit_dword(ctx, AARCH64_LSL(AARCH64_R0, AARCH64_R0, 24));
        jit_emit_dword(ctx, AARCH64_LSL(AARCH64_R1, AARCH64_R1, 24));
    } else if (opmode == 1) {
        jit_emit_dword(ctx, AARCH64_LSL(AARCH64_R0, AARCH64_R0, 16));
        jit_emit_dword(ctx, AARCH64_LSL(AARCH64_R1, AARCH64_R1, 16));
    }

    /* Execute arithmetic and capture 68k flags from NZCV. */
    jit_emit_dword(ctx, AARCH64_ADDS(AARCH64_R0, AARCH64_R0, AARCH64_R1));
    jit_emit_store_nzcv_flags(ctx, 0);

    /* Shift back and merge for byte/word destination updates. */
    if (opmode == 0) {
        jit_emit_dword(ctx, AARCH64_LSR(AARCH64_R0, AARCH64_R0, 24));
        jit_emit_load_dn(ctx, AARCH64_R2, dst_reg);
        jit_emit_mov64(ctx, AARCH64_R3, 0xFFFFFF00u);
        jit_emit_dword(ctx, AARCH64_AND(AARCH64_R2, AARCH64_R2, AARCH64_R3));
        jit_emit_dword(ctx, AARCH64_ORR(AARCH64_R0, AARCH64_R0, AARCH64_R2));
    } else if (opmode == 1) {
        jit_emit_dword(ctx, AARCH64_LSR(AARCH64_R0, AARCH64_R0, 16));
        jit_emit_load_dn(ctx, AARCH64_R2, dst_reg);
        jit_emit_mov64(ctx, AARCH64_R3, 0xFFFF0000u);
        jit_emit_dword(ctx, AARCH64_AND(AARCH64_R2, AARCH64_R2, AARCH64_R3));
        jit_emit_dword(ctx, AARCH64_ORR(AARCH64_R0, AARCH64_R0, AARCH64_R2));
    }
    ...
    if (dst_mode == 0) {
        ...
        if (src_mode == 0) {
            ...
            if (size == 0) {
                ...
                jit_emit_call_read16(ctx, AARCH64_R1, AARCH64_R1);
            } else {
                jit_emit_call_read32(ctx, AARCH64_R1, AARCH64_R1);
            }
        } else if (src_mode == 5) {
            /* SUB d16(An), Dn */
            int16_t disp = (int16_t)ext_words[0];
...
int jit_translate_cmp(jit_translate_context_t *tctx)
{
     ...
     *   001 CMP.W <ea>,Dn
     *   010 CMP.L <ea>,Dn
     *   011 CMPA.W / 111 CMPA.L (not handled here)
     */
    uint8_t opmode = (opcode >> 6) & 0x7;
    uint8_t dst_reg = (opcode >> 9) & 0x7;
    uint8_t src_mode = (opcode >> 3) & 0x7;
    ...
    /* CMP updates flags only. 68k subtract carry is inverse of AArch64 C. */
    jit_emit_dword(ctx, AARCH64_SUBS(AARCH64_R2, AARCH64_R0, AARCH64_R1));
    jit_emit_store_nzcv_flags(ctx, 1);
...
int jit_translate_bsr(jit_translate_context_t *tctx)
{
    ...
    /* Store updated SP before memory helper call (call clobbers caller-saved regs). */
    jit_emit_store_an(ctx, AARCH64_R0, 7);

    /* Push return address to stack */
    jit_emit_call_write32(ctx, AARCH64_R1, AARCH64_R0);
...
int jit_translate_misc(jit_translate_context_t *tctx)
{
    ...
    if ((opcode & 0xF0F8) == 0x50C8) {
        ...
        /*
         * DBcc semantics:
         * if condition TRUE: no decrement, fall through (PC+4)
         * if condition FALSE:
         *   decrement low 16-bit of Dn
         *   if result != 0xFFFF -> branch to PC+2+disp
         *   else fall through (PC+4)
         */

        /* Load CCR source fields and normalize to booleans. */
        jit_emit_load_cpu_reg(ctx, AARCH64_R2, off_n);
        jit_emit_load_cpu_reg(ctx, AARCH64_R3, off_notz);
        jit_emit_load_cpu_reg(ctx, AARCH64_R4, off_v);
        jit_emit_load_cpu_reg(ctx, AARCH64_R5, off_c);
        jit_emit_mov64(ctx, AARCH64_R1, 1);
        jit_emit_dword(ctx, AARCH64_LSR(AARCH64_R2, AARCH64_R2, 7));
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R2, AARCH64_R2, AARCH64_R1));
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R3, AARCH64_R3, AARCH64_R1)); /* !Z */
        jit_emit_dword(ctx, AARCH64_LSR(AARCH64_R4, AARCH64_R4, 7));
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R4, AARCH64_R4, AARCH64_R1));
        jit_emit_dword(ctx, AARCH64_LSR(AARCH64_R5, AARCH64_R5, 8));
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R5, AARCH64_R5, AARCH64_R1));

        /* cond_true bit -> R6 */
        switch (cond) {
            case 0x0: /* T */
                jit_emit_mov64(ctx, AARCH64_R6, 1);
                break;
            case 0x1: /* F */
                jit_emit_mov64(ctx, AARCH64_R6, 0);
                break;
            case 0x2: /* HI: !C && !Z */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R5, AARCH64_R1));
                jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R6, AARCH64_R6, AARCH64_R3));
                break;
            case 0x3: /* LS: C || Z */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R3, AARCH64_R1)); /* Z */
                jit_emit_dword(ctx, AARCH64_ORR_W(AARCH64_R6, AARCH64_R6, AARCH64_R5));
                break;
            case 0x4: /* CC */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R5, AARCH64_R1));
                break;
            case 0x5: /* CS */
                jit_emit_dword(ctx, AARCH64_ORR_W(AARCH64_R6, AARCH64_R5, AARCH64_R5));
                break;
            case 0x6: /* NE */
                jit_emit_dword(ctx, AARCH64_ORR_W(AARCH64_R6, AARCH64_R3, AARCH64_R3));
                break;
            case 0x7: /* EQ */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R3, AARCH64_R1));
                break;
            case 0x8: /* VC */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R4, AARCH64_R1));
                break;
            case 0x9: /* VS */
                jit_emit_dword(ctx, AARCH64_ORR_W(AARCH64_R6, AARCH64_R4, AARCH64_R4));
                break;
            case 0xA: /* PL */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R2, AARCH64_R1));
                break;
            case 0xB: /* MI */
                jit_emit_dword(ctx, AARCH64_ORR_W(AARCH64_R6, AARCH64_R2, AARCH64_R2));
                break;
            case 0xC: /* GE */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R2, AARCH64_R4));
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R6, AARCH64_R1));
                break;
            case 0xD: /* LT */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R2, AARCH64_R4));
                break;
            ...
            case 0xF: /* LE */
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R2, AARCH64_R4));
                jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R7, AARCH64_R3, AARCH64_R1)); /* Z */
                jit_emit_dword(ctx, AARCH64_ORR_W(AARCH64_R6, AARCH64_R6, AARCH64_R7));
                break;
            default:
                return -1;
        }

        /* Decrement low word of Dn and store back. */
        jit_emit_load_dn(ctx, AARCH64_R0, dn);
        jit_emit_mov64(ctx, AARCH64_R1, 0xFFFF);
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R2, AARCH64_R0, AARCH64_R1)); /* low16 */
        jit_emit_mov64(ctx, AARCH64_R4, 1);
        jit_emit_dword(ctx, AARCH64_SUB(AARCH64_R2, AARCH64_R2, AARCH64_R4));
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R2, AARCH64_R2, AARCH64_R1));
        jit_emit_mov64(ctx, AARCH64_R4, 0xFFFF0000);
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R0, AARCH64_R0, AARCH64_R4)); /* high16 */
        jit_emit_dword(ctx, AARCH64_ORR_W(AARCH64_R0, AARCH64_R0, AARCH64_R2));
        jit_emit_store_dn(ctx, AARCH64_R0, dn);

        /* ctr_is_minus1 bit in R4 (1 when low16 == 0xFFFF). */
        jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R7, AARCH64_R2, AARCH64_R1));
        jit_emit_mov64(ctx, AARCH64_R4, 0);
        jit_emit_dword(ctx, AARCH64_CBNZ(AARCH64_R7, 8));
        jit_emit_mov64(ctx, AARCH64_R4, 1);

        /* taken = (!cond_true) & (!ctr_is_minus1) -> R6 */
        jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R6, AARCH64_R6, AARCH64_R1));
        jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R4, AARCH64_R4, AARCH64_R1));
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R6, AARCH64_R6, AARCH64_R4));

        /* Select next PC: taken ? (pc+2+disp) : (pc+4). */
        jit_emit_load_pc(ctx, AARCH64_R0);
        jit_emit_mov64(ctx, AARCH64_R1, 4);
        jit_emit_dword(ctx, AARCH64_ADD(AARCH64_R1, AARCH64_R0, AARCH64_R1)); /* fallthrough */
        jit_emit_mov64(ctx, AARCH64_R2, (uint64_t)(int64_t)(2 + disp));
        jit_emit_dword(ctx, AARCH64_ADD(AARCH64_R2, AARCH64_R0, AARCH64_R2)); /* taken */
        jit_emit_mov64(ctx, AARCH64_R3, 0);
        jit_emit_dword(ctx, AARCH64_SUB(AARCH64_R6, AARCH64_R3, AARCH64_R6)); /* mask = -taken */
        jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R4, AARCH64_R1, AARCH64_R2));
        jit_emit_dword(ctx, AARCH64_AND_W(AARCH64_R4, AARCH64_R4, AARCH64_R6));
        jit_emit_dword(ctx, AARCH64_EOR_W(AARCH64_R1, AARCH64_R1, AARCH64_R4));
        jit_emit_store_pc(ctx, AARCH64_R1);
