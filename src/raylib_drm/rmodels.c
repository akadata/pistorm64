...
// static ModelAnimation *LoadModelAnimationGLTF(const char *fileName, unsigned int *animCount); //
...
// NOTE: It could be also used for pyramid and cone
void DrawCylinderEx(Vector3 startPos, Vector3 endPos, float startRadius, float endRadius, int sides,
                    Color color) {
  if (sides < 3)
    sides = 3;
  ...
  for (int i = 0; i < sides; i++) {
    ...
    Vector3 w3 = {endPos.x + s3 * b1.x + c3 * b2.x, endPos.y + s3 * b1.y + c3 * b2.y,
                  endPos.z + s3 * b1.z + c3 * b2.z};
    float s4 = sinf(baseAngle * (i + 1)) * endRadius;
    float c4 = cosf(baseAngle * (i + 1)) * endRadius;
    Vector3 w4 = {endPos.x + s4 * b1.x + c4 * b2.x, endPos.y + s4 * b1.y + c4 * b2.y,
...
// NOTE: It could be also used for pyramid and cone
void DrawCylinderWiresEx(Vector3 startPos, Vector3 endPos, float startRadius, float endRadius,
                         int sides, Color color) {
  if (sides < 3)
    sides = 3;
...
#endif
#if defined(SUPPORT_FILEFORMAT_GLTF)
    // if (IsFileExtension(fileName, ".gltf;.glb")) animations = LoadModelAnimationGLTF(fileName,
    // animCount);
#endif
    ...
    for (int m = 0; m < model.meshCount; m++) {
      ...
      for (int vCounter = 0; vCounter < vValues; vCounter += 3) {
        ...
        for (int j = 0; j < 4; j++, boneCounter++) {
          ...
          animVertex = (Vector3){mesh.vertices[vCounter], mesh.vertices[vCounter + 1],
          ...
          // normals)
          if (mesh.normals != NULL) {
            animNormal = (Vector3){mesh.normals[vCounter], mesh.normals[vCounter + 1],
...
#define MATERIAL_NAME_LENGTH 32 // Material name string length
  ...
  */
  ...
  for (int i = 0; i < model.meshCount; i++) {
    ...
    model.meshes[i].boneIds = RL_CALLOC(model.meshes[i].vertexCount * 4,
                                        sizeof(unsigned char)); // Up-to 4 bones supported!
...
static Image LoadImageFromCgltfImage(cgltf_image* cgltfImage, const char* texPath) {
  ...
  if (cgltfImage->uri != NULL) // Check if image data is provided as a uri (base64 or path)
  {
    if ((strlen(cgltfImage->uri) > 5) && (cgltfImage->uri[0] == 'd') &&
        (cgltfImage->uri[1] == 'a') && (cgltfImage->uri[2] == 't') && (cgltfImage->uri[3] == 'a') &&
  ...
  } else if (cgltfImage->buffer_view->buffer->data !=
      ...
      for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++) {
              ...
              cgltf_attribute_type_position) // POSITION
          {
            ...
            if ((attribute->component_type == cgltf_component_type_r_32f) &&
              ...
              TRACELOG(LOG_WARNING,
                       ...
                       fileName);
          } else if (data->meshes[i].primitives[p].attributes[j].type ==
                     ...
                     cgltf_attribute_type_texcoord) // TEXCOORD_0
          {
            ...
            if ((attribute->component_type == cgltf_component_type_r_32f) &&
              ...
              TRACELOG(LOG_WARNING,
                     ...
                     cgltf_attribute_type_color) // COLOR_0
          {
            cgltf_accessor* attribute = data->meshes[i].primitives[p].attributes[j].data;
            ...
            if ((attribute->component_type == cgltf_component_type_r_8u) &&
              ...
              TRACELOG(LOG_WARNING, "MODEL: [%s] Color attribute data format not supported",
                       fileName);
          }
        ...
        if (data->meshes[i].primitives[p].indices != NULL) {
          ...
          if (attribute->component_type == cgltf_component_type_r_16u) {
            ...
            TRACELOG(LOG_WARNING, "MODEL: [%s] Indices data format not supported, use u16",
                     fileName);
        } else
