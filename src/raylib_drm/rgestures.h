...
void ProcessGestureEvent(GestureEvent event) {
  ...
  if (GESTURES.Touch.pointCount == 1) // One touch point
  {
    if (event.touchAction == TOUCH_ACTION_DOWN) {
      ...
      GESTURES.Drag.distance =
          rgVector2Distance(GESTURES.Touch.downPositionA, GESTURES.Touch.upPosition);
      GESTURES.Drag.intensity =
          GESTURES.Drag.distance / (float)((rgGetCurrentTime() - GESTURES.Swipe.timeDuration));
      ...
      if ((GESTURES.Drag.intensity > FORCE_TO_SWIPE) &&
        ...
        GESTURES.Drag.angle =
            360.0f - rgVector2Angle(GESTURES.Touch.downPositionA, GESTURES.Touch.upPosition);
      ...
      if (GESTURES.current == GESTURE_HOLD) {
        ...
        // Detect GESTURE_DRAG
        if (rgVector2Distance(GESTURES.Touch.downPositionA, GESTURES.Touch.moveDownPositionA) >=
            MINIMUM_DRAG) {
          GESTURES.Touch.eventTime = rgGetCurrentTime();
  ...
  } else if (GESTURES.Touch.pointCount == 2) // Two touch points
  {
    if (event.touchAction == TOUCH_ACTION_DOWN) {
      ...
      // GESTURES.Pinch.distance = rgVector2Distance(GESTURES.Touch.downPositionA,
      ...
      if ((rgVector2Distance(GESTURES.Touch.downPositionA, GESTURES.Touch.moveDownPositionA) >=
           MINIMUM_PINCH) ||
          (rgVector2Distance(GESTURES.Touch.downPositionB, GESTURES.Touch.moveDownPositionB) >=
           MINIMUM_PINCH)) {
      ...
      // NOTE: Angle should be inverted in Y
      GESTURES.Pinch.angle = 360.0f - rgVector2Angle(GESTURES.Touch.moveDownPositionA,
...
void UpdateGestures(void) {
  ...
  if (((rgGetCurrentTime() - GESTURES.Touch.eventTime) > TAP_TIMEOUT) &&
