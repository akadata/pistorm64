...
Wave LoadWaveFromMemory(const char* fileType, const unsigned char* fileData, int dataSize) {
  ...
  if (false) {
    ...
    if (oggData != NULL) {
      ...
      wave.frameCount =
          (unsigned int)stb_vorbis_stream_length_in_samples(oggData); // NOTE: It returns frames!
      wave.data = (short*)RL_MALLOC(wave.frameCount * wave.channels * sizeof(short));

      // NOTE: Get the number of samples to process (be careful! we ask for number of shorts, not
      // bytes!)
...
bool ExportWave(Wave wave, const char* fileName) {
  ...
  if (false) {
    ...
    success =
        SaveFileData(fileName, wave.data, wave.frameCount * wave.channels * wave.sampleSize / 8);
  }
...
#define TEXT_BYTES_PER_LINE 20
  ...
  // NOTE: Frame data exported is interlaced: Frame01[Sample-Channel01, Sample-Channel02, ...],
  // Frame02[], Frame03[]
  byteCount += sprintf(txtData + byteCount, "static unsigned char %s_DATA[%i] = { ", varFileName,
                       waveDataSize);
...
Music LoadMusicStream(const char* fileName) {
  ...
  if (false) {
    ...
    if (result == 0) // XM AUDIO.System.context created successfully
    {
      ...
      music.frameCount =
          (unsigned int)jar_xm_get_remaining_samples(ctxXm); // NOTE: Always 2 channels (stereo)
      music.looping = true;                                  // Looping enabled by default
    ...
    if (result > 0) {
      ...
      music.frameCount =
          (unsigned int)jar_mod_max_samples(ctxMod); // NOTE: Always 2 channels (stereo)
      music.looping = true;                          // Looping enabled by default
      musicLoaded = true;
    }
  }
...
void UpdateMusicStream(Music music) {
  ...
  // TODO: Get the framesLeft using framesProcessed... but first, get total frames processed
  // correctly...
  ...
  while (IsAudioStreamProcessed(music.stream)) {
    ...
    case MUSIC_AUDIO_FLAC: {
      // NOTE: Returns the number of samples to process (not required)
      drflac_read_pcm_frames_s16((drflac*)music.ctxData, frameCountToStream * music.stream.channels,
                                 (short*)pcm);
  ...
  if (stream.buffer != NULL) {
    if (stream.buffer->isSubBufferProcessed[0] || stream.buffer->isSubBufferProcessed[1]) {
      ...
      if (subBufferSizeInFrames >= (ma_uint32)frameCount) {
        ...
        if (framesToWrite > (ma_uint32)frameCount)
          framesToWrite = (ma_uint32)frameCount;
