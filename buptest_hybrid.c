...
int main(int argc, char *argv[]) {
    ...
    // HYBRID SETUP: Initialize protocol WITHOUT GPCLK setup
    // In hybrid setup, Pi Zero 2W provides 200MHz clock, Pi5 handles signals only
    // We'll set up GPIO but not touch the clock
    printf("PiStorm Hybrid Setup: Pi Zero 2W provides 200MHz clock\n");
    printf("Pi5 handles data/signals only (no GPCLK setup)\n");
    
    // Set environment to not touch GPCLK
    setenv("PISTORM_ENABLE_GPCLK", "0", 1);
    setenv("PISTORM_RP1_LEAVE_CLK_PIN", "1", 1);
    
    ps_setup_protocol();  // This will set up GPIO but not GPCLK in hybrid mode
    ps_reset_state_machine();
    ps_pulse_reset();
