...
```bash
mkdir -p /tank/pistorm/ipc captures
```
...
socat -d -d UNIX-LISTEN:/tank/pistorm/ipc/amiga.sock,fork \
  SYSTEM:'ts "[%Y-%m-%d %H:%M:%S]" | tee -a /tank/pistorm/ipc/amiga_markers.log'
```

**Step B: On amiga, create reverse tunnel**:
```bash
ssh -N -R 9009:/tank/pistorm/ipc/amiga.sock <homer_ip_address>
```
...
#!/usr/bin/env bash
set -euo pipefail
exec ssh -N -R 9009:/tank/pistorm/ipc/amiga.sock <homer_ip_address>
```
...
- Check file permissions in /tank/pistorm/ipc/
