...
# -------------------------
# Profiling / PGO support
#
# Targets:
#   make profile      -> instrumented build (generates profile data)
#   make runprofile   -> runs emulator to collect profile data
#   make buildprofile -> rebuild using collected profile data (PGO)
#
# PGO_MODE:
#   gen -> build with instrumentation
#   use -> build using collected profile data

PGO_DIR   ?= .pgo
PGO_MODE  ?=

# Build-time compile profiling (clang: JSON traces, gcc: text report)
BUILD_TRACE ?= 0
...
ifeq ($(findstring clang,$(CC)),clang)
  # Clang/LLVM PGO
  ifeq ($(PGO_MODE),gen)
    PROFILE_CFLAGS  += -fprofile-instr-generate
    PROFILE_LDFLAGS += -fprofile-instr-generate
  endif
  ifeq ($(PGO_MODE),use)
    PROFILE_CFLAGS  += -fprofile-instr-use=$(PGO_DIR)/pgo.profdata
    PROFILE_LDFLAGS += -fprofile-instr-use=$(PGO_DIR)/pgo.profdata
  endif

  ifeq ($(BUILD_TRACE),1)
    TRACE_CFLAGS += -ftime-trace
  endif
else
  # GCC PGO
  ifeq ($(PGO_MODE),gen)
    PROFILE_CFLAGS  += -fprofile-generate=$(PGO_DIR)
    PROFILE_LDFLAGS += -fprofile-generate=$(PGO_DIR)
  endif
  ifeq ($(PGO_MODE),use)
    PROFILE_CFLAGS  += -fprofile-use=$(PGO_DIR) -fprofile-correction
    PROFILE_LDFLAGS += -fprofile-use=$(PGO_DIR) -fprofile-correction
  endif

  ifneq (,$(filter gen use,$(PGO_MODE)))
    PROFILE_LDLIBS += -lgcov
  endif

  ifeq ($(BUILD_TRACE),1)
    TRACE_CFLAGS += -ftime-report
  endif
endif
...
CFLAGS       = $(EMU_WARNINGS) $(OPT_LEVEL) $(CPUFLAGS) $(DEFINES) $(INCLUDES) $(ACFLAGS) $(LTO_FLAGS) $(PLT_FLAGS) $(FP_FLAGS) $(PIPE_FLAGS) $(PROFILE_CFLAGS) $(TRACE_CFLAGS) $(EXTRA_CFLAGS)
CXXFLAGS     = $(CXX_WARNINGS) $(OPT_LEVEL) $(CPUFLAGS) $(DEFINES) $(INCLUDES) $(LTO_FLAGS) $(PLT_FLAGS) $(FP_FLAGS) $(PIPE_FLAGS) $(PROFILE_CFLAGS) $(TRACE_CFLAGS) $(EXTRA_CFLAGS)
M68K_CFLAGS   = $(WARNINGS) $(OPT_LEVEL) $(CPUFLAGS) $(DEFINES) $(INCLUDES) $(ACFLAGS) $(LTO_FLAGS) $(PLT_FLAGS) $(FP_FLAGS) $(PIPE_FLAGS) $(PROFILE_CFLAGS) $(TRACE_CFLAGS) $(M68K_WARN_SUPPRESS) $(EXTRA_M68K_CFLAGS)
LDFLAGS      = $(WARNINGS) $(LD_GOLD) $(LDSEARCH) $(LTO_FLAGS) $(PROFILE_LDFLAGS) $(EXTRA_LDFLAGS)

LDLIBS   = $(RAYLIB_LIBS) $(LIBS) $(LDLIBS_VC) $(LDLIBS_ALSA) $(PROFILE_LDLIBS)
...
buildprofile:
	@mkdir -p $(PGO_DIR)
	@if [ "$(findstring clang,$(CC))" != "" ]; then \
		command -v llvm-profdata >/dev/null 2>&1 || { echo "ERROR: llvm-profdata missing (needed for clang PGO)."; exit 1; }; \
		ls -1 $(PGO_DIR)/*.profraw >/dev/null 2>&1 || { echo "ERROR: no .profraw files in $(PGO_DIR) (run 'make runprofile')."; exit 1; }; \
		echo "[PGO] Merging profraw -> $(PGO_DIR)/pgo.profdata"; \
		llvm-profdata merge -o $(PGO_DIR)/pgo.profdata $(PGO_DIR)/*.profraw; \
	fi
	$(MAKE) -f Makefile.profile clean
...
.PHONY: all clean buptest pistorm_truth_test install uninstall \
