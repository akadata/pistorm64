...
static uint64_t pistorm_ami_read(void* opaque, hwaddr addr, unsigned size) {
  uint64_t value;
  ...
  switch (size) {
  case 1:
    value = (uint64_t)read8((uint32_t)addr);
    break;
  case 2:
    value = (uint64_t)read16((uint32_t)addr);
    break;
  case 4:
    value = (uint64_t)read32((uint32_t)addr);
    break;
  default:
    value = 0x4E71;
    printf("WTF READ?!\n");
...
static void pistorm_ami_write(void* opaque, hwaddr addr, uint64_t val, unsigned size) {
  //   printf("WRITE SIZE:%x ADDR:%llx DATA:%llx\n",size,addr,val);
  switch (size) {
  case 1:
    write8((uint32_t)addr, (uint32_t)val);
    break;
  case 2:
    write16((uint32_t)addr, (uint32_t)val);
    break;
  case 4:
    write32((uint32_t)addr, (uint32_t)val);
    break;
  default:
    printf("WTF WRITE?!\n");
    break;
