...
uint32 DECODE_EA_32(m68ki_cpu_core* state, int ea) {
  ...
  case 7: {
    ...
    default:
      fatalerror("m68k: DECODE_EA_32: unhandled mode %d, reg %d at %08X\n", mode, reg, REG_PC);
    }
    break;
  }
  default:
...
#ifdef PISTORM_PMMU_LOG
    MMUERR(("[PMMU] buserror addr=0x%08X sr=0x%04X rw=%u fc=%u sz=%u\n", addr_in, state->mmu_tmp_sr,
            state->mmu_tmp_rw, state->mmu_tmp_fc, state->mmu_tmp_sz));
#endif
  }
}
...
void pmmu_atc_add(m68ki_cpu_core* state, uint32 logical, uint32 physical, int fc, int rw) {
  ...
  if (state->mmu_tmp_sr &
      (M68K_MMU_SR_BUS_ERROR | M68K_MMU_SR_INVALID | M68K_MMU_SR_SUPERVISOR_ONLY)) {
...
// template<bool ptest>
uint16 pmmu_atc_lookup(m68ki_cpu_core* state, uint32 addr_in, int fc, uint16 rw, uint32* addr_out,
                       int ptest) {
...
void update_descriptor(m68ki_cpu_core* state, uint32 tptr, int type, uint32 entry, int16 rw) {
  ...
  if (type == M68K_MMU_DF_DT_PAGE && !rw && !(entry & M68K_MMU_DF_MODIFIED) &&
...
// template<bool ptest>
uint16 pmmu_walk_tables(m68ki_cpu_core* state, uint32 addr_in, int type, uint32 table, uint8 fc,
  ...
  do {
    ...
    MMULOG(("%s: type %d, table %08x, addr_in %08x, indexbits %d, pageshift %d, indirect %d "
            "table_index %08x, rw=%d fc=%d\n",
...
uint32 pmmu_translate_addr_with_fc(m68ki_cpu_core* state, uint32 addr_in, uint8 fc, uint16 rw,
                                   int limit, int ptest, int pload) {
  uint32 addr_out = 0;

  MMULOG(("%s: addr_in=%08x, fc=%d, ptest=%d, rw=%d, limit=%d, pload=%d\n", __func__, addr_in, fc,
          ptest, rw, limit, pload));
  state->mmu_tmp_sr = 0;

  state->mmu_last_logical_addr = addr_in;

  if (pmmu_match_tt(state, addr_in, fc, state->mmu_tt0, rw) ||
      pmmu_match_tt(state, addr_in, fc, state->mmu_tt1, rw) || fc == 7) {
    return addr_in;
  }
  ...
  if (!ptest && !pload && pmmu_atc_lookup(state, addr_in, fc, rw, &addr_out, 0)) {
    if ((state->mmu_tmp_sr & M68K_MMU_SR_BUS_ERROR) ||
      ...
      MMULOG(("set atc hit buserror: addr_in=%08x, addr_out=%x, rw=%x, fc=%d, sz=%d\n", addr_in,
              addr_out, state->mmu_tmp_rw, state->mmu_tmp_fc, state->mmu_tmp_sz));
  ...
  if (!pmmu_walk_tables(state, addr_in, type, tbl_addr, fc, limit, rw, &addr_out, ptest)) {
    MMULOG(
        ("%s: addr_in=%08x, type=%x, tbl_addr=%x, fc=%d, limit=%x, rw=%x, addr_out=%x, ptest=%d\n",
...
// the 68040 is a subset of the 68851 and 68030 PMMUs - the page table sizes are fixed, there is no
// early termination, etc, etc.
uint32 pmmu_translate_addr_with_fc_040(m68ki_cpu_core* state, uint32 addr_in, uint8 fc,
                                       uint8 ptest) {
  ...
  // they don't do much in emulation because we never write out of order, but the write-protect and
  // cache control features are emulatable, and apparently transparent translation regions skip the
  // page table lookup.
  ...
  if (state->pmmu_enabled) {
    ...
    if (root_entry & 2) {
      ...
      if ((((root_entry & 4) && !state->mmu_tmp_rw) ||
           ((pointer_entry & 4) && !state->mmu_tmp_rw)) &&
          !ptest) {
        pmmu_set_buserror(state, addr_in);
        return addr_in;
      }
    ...
    if ((((page_entry & 4) && !state->mmu_tmp_rw) || ((page_entry & 0x80) && !(fc & 4))) &&
        !ptest) {
      pmmu_set_buserror(state, addr_in);
      return addr_in;
    }
...
uint32 pmmu_translate_addr(m68ki_cpu_core* state, uint32 addr_in, uint16 rw) {
  ...
  if (state->mmu_tmp_sr & (M68K_MMU_SR_BUS_ERROR | M68K_MMU_SR_INVALID |
    ...
    MMUERR(("[PMMU] translate addr=0x%08X -> 0x%08X sr=0x%04X rw=%u fc=%u\n", addr_in, addr_out,
            state->mmu_tmp_sr, rw, state->mmu_tmp_fc));
  }
#endif
  return addr_out;
}
...
void m68851_ptest(m68ki_cpu_core* state, uint32 ea, uint16 modes) {
  ...
  MMULOG(("PMMU: PTEST%c (%04X) pc=%08x sp=%08x va=%08x fc=%x level=%x a=%d, areg=%d\n",
          rw ? 'R' : 'W', modes, state->ppc, REG_A[7], v_addr, fc, level, (modes & 0x100) ? 1 : 0,
          (modes >> 5) & 7));
...
void m68851_pmove(m68ki_cpu_core* state, uint32 ea, uint16 modes) {
  ...
  case 2: // MC68851 form, FD never set
    if (modes & 0x200) {
      ...
      break;
    } else // top 3 bits of modes: 010 for this, 011 for status, 000 for transparent translation
           // regs
    {
...
inline uint32 hmmu_translate_addr(uint32 addr_in)
{
  ...
  else if (state->hmmu_enabled == M68K_HMMU_ENABLE_II) // the original II does a more complex
translation
  {
    addr_out = addr_in & 0xffffff;
