...
#if defined(PLATFORM_WEB)
static EM_BOOL EmscriptenMouseCallback(int eventType, const EmscriptenMouseEvent* mouseEvent,
                                       void* userData);
...
RLAPI Shader LoadShaderFromMemory(const char* vsCode, const char* fsCode) {
  ...
  if (shader.id > 0) {
    ...
    shader.locs[SHADER_LOC_MAP_DIFFUSE] = rlGetLocationUniform(
        shader.id, RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE0); // SHADER_LOC_MAP_ALBEDO
    shader.locs[SHADER_LOC_MAP_SPECULAR] = rlGetLocationUniform(
        shader.id, RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE1); // SHADER_LOC_MAP_METALNESS
...
static int32_t AndroidInputCallback(struct android_app* app, AInputEvent* event) {
  ...
  for (int i = 0; (i < CORE.Input.Touch.pointCount) && (i < MAX_TOUCH_POINTS); i++) {
    ...
    CORE.Input.Touch.position[i] =
        (Vector2){AMotionEvent_getX(event, i), AMotionEvent_getY(event, i)};
...
static EM_BOOL EmscriptenTouchCallback(int eventType, const EmscriptenTouchEvent* touchEvent,
  ...
  for (int i = 0; (i < CORE.Input.Touch.pointCount) && (i < MAX_TOUCH_POINTS); i++) {
    ...
    // Register touch points position
    CORE.Input.Touch.position[i] =
        (Vector2){touchEvent->touches[i].targetX, touchEvent->touches[i].targetY};
