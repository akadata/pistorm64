...
#define ADDRESS_INFO_DNS_ONLY                                                                      \
  (0x00000010) // Restrict queries to unicast DNS only (no LLMNR, netbios, etc.)
...
#define ADDRESS_INFO_DISABLE_IDN_ENCODING                                                          \
  (0x00080000)                             // Disable Internationalized Domain Names handling
...
typedef struct UDPChannel {
  ...
  IPAddress
      address[SOCKET_MAX_UDPADDRESSES]; // The list of remote addresses this channel is bound to
} UDPChannel;
...
typedef struct Socket {
  ...
  struct UDPChannel
      binding[SOCKET_MAX_UDPCHANNELS]; // The amount of channels (if UDP) this socket is bound to
} Socket;
...
int ResolveHost(const char* address, const char* service, int addressType, int flags,
...
//----------------------------------------------------------------------------------
static void PrintSocket(struct sockaddr_storage* addr, const int family, const int socktype,
                        const int protocol);
...
static bool InitSocket(Socket* sckt, struct addrinfo* address) {
  ...
  case SOCKET_TCP: {
    if (address->ai_family == AF_INET)
      sckt->channel = socket(AF_INET, SOCK_STREAM, 0);
    else
      sckt->channel = socket(AF_INET6, SOCK_STREAM, 0);
  } break;
  case SOCKET_UDP: {
    if (address->ai_family == AF_INET)
      sckt->channel = socket(AF_INET, SOCK_DGRAM, 0);
    else
      sckt->channel = socket(AF_INET6, SOCK_DGRAM, 0);
  } break;
  default:
...
static bool CreateSocket(SocketConfig* config, SocketResult* outresult) {
  ...
  //
  //    AI_CANONNAME    Causes the ai_canonname of the result to the filled  out with the host's
  //    canonical (real) name. AI_PASSIVE:        Causes the result's IP address to be filled out
  //    with INADDR_ANY (IPv4)or in6addr_any (IPv6); Note:    This causes a subsequent call to
  //    bind() to auto-fill the IP address
  //            of the struct sockaddr with the address of the current host.
  //
  SocketSetHints(config, &hints);

  // Populate address information
  addrstatus = getaddrinfo(
      config->host, // e.g. "www.example.com" or IP (Can be null if AI_PASSIVE flag is set
  ...
  if (addrstatus != 0) {
    ...
    TRACELOG(LOG_WARNING, "Failed to get resolve host %s:%s: %s", config->host, config->port,
    ...
    // int rc = getnameinfo((struct sockaddr *)res->ai_addr, client_len, hoststr, sizeof(hoststr),
  ...
  if (success) {
    ...
    case AF_INET: {
      ...
      if (outresult->socket->addripv4 != NULL) {
        ...
        if (outresult->socket->addripv4 != NULL) {
          memcpy(&outresult->socket->addripv4->address, (struct sockaddr_in*)res->ai_addr,
          ...
          socklen_t client_len = sizeof(struct sockaddr_storage);
          getnameinfo((struct sockaddr*)&outresult->socket->addripv4->address, client_len, hoststr,
                      sizeof(hoststr), portstr, sizeof(portstr), NI_NUMERICHOST | NI_NUMERICSERV);
    ...
    case AF_INET6: {
      outresult->socket->addripv6 =
          (struct _SocketAddressIPv6*)RNET_MALLOC(sizeof(*outresult->socket->addripv6));
      if (outresult->socket->addripv6 != NULL) {
        memset(outresult->socket->addripv6, 0, sizeof(*outresult->socket->addripv6));
        if (outresult->socket->addripv6 != NULL) {
          memcpy(&outresult->socket->addripv6->address, (struct sockaddr_in6*)res->ai_addr,
                 sizeof(struct sockaddr_in6));
          outresult->socket->isIPv6 = true;
          char hoststr[NI_MAXHOST];
          char portstr[NI_MAXSERV];
          socklen_t client_len = sizeof(struct sockaddr_storage);
          getnameinfo((struct sockaddr*)&outresult->socket->addripv6->address, client_len, hoststr,
                      sizeof(hoststr), portstr, sizeof(portstr), NI_NUMERICHOST | NI_NUMERICSERV);
          TRACELOG(LOG_INFO, "Socket address set to %s:%s", hoststr, portstr);
        }
      }
...
void ResolveIP(const char* ip, const char* port, int flags, char* host, char* serv) {
  ...
  case AF_INET: {
    status = getnameinfo(&*((struct sockaddr*)res->ai_addr),
                         sizeof(*((struct sockaddr_in*)res->ai_addr)), host, NI_MAXHOST, serv,
                         NI_MAXSERV, flags);
  } break;
  case AF_INET6: {
    /*
...
//
//  Parameters:
//      const char* address - A pointer to a NULL-terminated ANSI string that contains a host (node)
//      name or a numeric host address string. const char* service - A pointer to a NULL-terminated
//      ANSI string that contains either a service name or port number represented as a string.
//
//    Returns:
  ...
  for (iterator = res; iterator != NULL; iterator = iterator->ai_next) {
    if (i < size) {
      ...
      *outAddr[i]->addr.ai_addr = *iterator->ai_addr;
      ...
      // PrintSocket(outAddr[i]->addr.ai_addr, outAddr[i]->addr.ai_family,
      // outAddr[i]->addr.ai_socktype, outAddr[i]->addr.ai_protocol);
...
// Note: The bind function is required on an unconnected socket before subsequent calls to the
// listen function.
bool SocketBind(SocketConfig* config, SocketResult* result) {
  bool success = false;
  ...
  if (!IsSocketValid(result->socket) || !config->server) {
    ...
    if (sock_addr != NULL) {
      if (bind(result->socket->channel, (struct sockaddr*)sock_addr, sizeof(*sock_addr)) !=
          SOCKET_ERROR) {
  ...
  if (success) {
    ...
    if (getsockname(result->socket->channel, (struct sockaddr*)sock_addr, &sock_len) < 0) {
      ...
      memcpy(&result->socket->addripv4->address, (struct sockaddr_in*)&s->sin_addr,
             sizeof(struct sockaddr_in));
    }
  }
  return success;
...
bool SocketConnect(SocketConfig* config, SocketResult* result) {
  ...
  if (config->server) {
    ...
    if (IsIPv4Address(config->host)) {
      ...
      // TODO: Changed the code to avoid the usage of inet_pton and inet_ntop replacing them with
      // getnameinfo (that should have a better support on windows).

      // inet_pton(AF_INET, config->host, &ip4addr.sin_addr);
      int connect_result =
          connect(result->socket->channel, (struct sockaddr*)&ip4addr, sizeof(ip4addr));
      ...
      if (IsIPv6Address(config->host)) {
        ...
        int connect_result =
            connect(result->socket->channel, (struct sockaddr*)&ip6addr, sizeof(ip6addr));
...
Socket* SocketAccept(Socket* server, SocketConfig* config) {
  ...
  case AF_INET: {
    ...
    if (sock->addripv4 != NULL) {
      ...
      memcpy(&sock->addripv4->address, (struct sockaddr_in*)&s->sin_addr,
             sizeof(struct sockaddr_in));
      TRACELOG(LOG_INFO, "Server: Got connection from %s::%hu",
               SocketAddressToString((struct sockaddr_storage*)s),
  ...
  case AF_INET6: {
    ...
    if (sock->addripv6 != NULL) {
      ...
      memcpy(&sock->addripv6->address, (struct sockaddr_in6*)&s->sin6_addr,
             sizeof(struct sockaddr_in6));
      TRACELOG(LOG_INFO, "Server: Got connection from %s::%hu",
               SocketAddressToString((struct sockaddr_storage*)s),
...
// Allocate/Free a UDP packet vector (array of packets) of 'howmany' packets, each 'size' bytes
// long. A pointer to the packet array is returned, or NULL if the function ran out of memory.
  ...
  case SOCKET_UDP: {
    ...
    if (sock->isIPv6)
      status = sendto(sock->channel, (const char*)data, left, 0,
                      (struct sockaddr*)&sock->addripv6->address, sizeof(sock->addripv6->address));
    else
      status = sendto(sock->channel, (const char*)data, left, 0,
                      (struct sockaddr*)&sock->addripv4->address, sizeof(sock->addripv4->address));
  ...
  case SOCKET_TCP: {
    ...
    if (len > 0) {
      ...
      if (sock->type == SOCKET_UDP) {
        // TRACELOG(LOG_DEBUG, "Received data from: %s", inet_ntop(sock_addr.ss_family,
        // GetSocketAddressPtr((struct sockaddr *)&sock_addr), ip, sizeof(ip)));
      }
...
bool IsSocketConnected(Socket* sock) {
  ...
  else if (FD_ISSET(sock->channel, &writefds))
    return true;
...
AddressInformation LoadAddress(void) {
  ...
  if (addressInfo != NULL) {
    ...
    if (addressInfo->addr.ai_addr == NULL)
      TRACELOG(LOG_WARNING, "Failed to allocate memory for \"struct sockaddr\"");
  } else
    TRACELOG(LOG_WARNING, "Failed to allocate memory for \"struct AddressInformation\"");
