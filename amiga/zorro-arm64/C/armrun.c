...
static int run_one_payload(struct Library *lib_base, const char *path) {
  ...
  printf("%s: ok result0=$%08X result1=$%08X job_state=%u job_result=%u retval=$%08X%08X svc=%u hook=$%08X\n", path,
         ...
         (unsigned int)result.service_dispatch_count, (unsigned int)result.service_hook_status);
  return 0;
}
