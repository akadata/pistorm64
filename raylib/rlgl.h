...
#define DEFAULT_BATCH_DRAWCALLS                                                                    \
...
#define MAX_BATCH_ACTIVE_TEXTURES                                                                  \
  4 // Maximum number of additional textures that can be activated on batch drawing
...
typedef struct DrawCall {
  ...
  // draw -> Using RLGL.currentShader.id
  unsigned int
      textureId; // Texture id to be used on the draw -> Use to create new draw call if changes

  // Matrix projection;        // Projection matrix for this draw -> Using RLGL.projection by
  // default Matrix modelview;         // Modelview matrix for this draw -> Using RLGL.modelview by
  // default
} DrawCall;
...
RLAPI void rlRotatef(float angleDeg, float x, float y,
...
RLAPI RenderBatch rlLoadRenderBatch(int numBuffers,
...
RLAPI void rlSetRenderBatchActive(
    RenderBatch* batch); // Set the active render batch for rlgl (NULL for default internal)
...
RLAPI void rlUpdateVertexBuffer(int bufferId, void* data, int dataSize,
...
typedef struct rlglData {
  ...
  struct {
    ...
    unsigned int activeTextureId[MAX_BATCH_ACTIVE_TEXTURES]; // Active texture ids to be enabled on
    ...
    if (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].vertexCount > 0) {
      ...
      if (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].mode == RL_LINES)
        ...
        RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].vertexAlignment =
            ((RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].vertexCount < 4)
                 ? 1
                 : (4 - (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].vertexCount %
                         4)));
      else
...
void rlVertex3f(float x, float y, float z) {
  ...
  if (RLGL.State.transformRequired)
    vec = Vector3Transform(vec, RLGL.State.transform);
...
void rlTextureParameters(unsigned int id, int param, int value) {
  ...
  case RL_TEXTURE_FILTER_ANISOTROPIC: {
    ...
    if (value <= RLGL.ExtSupported.maxAnisotropyLevel)
      ...
      TRACELOG(LOG_WARNING, "GL: Maximum anisotropic filter level supported is %iX", id,
...
void rlglInit(int width, int height) {
  ...
  if (RLGL.State.defaultTextureId != 0)
    TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Default texture loaded successfully",
             RLGL.State.defaultTextureId);
  else
...
void rlglClose(void) {
  ...
  TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Default texture unloaded successfully",
           RLGL.State.defaultTextureId);
#endif
}
...
#define GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT 0x84FF
  ...
  if (RLGL.ExtSupported.texAnisoFilter)
    TRACELOG(LOG_INFO, "    GL_MAX_TEXTURE_MAX_ANISOTROPY: %.0f",
             RLGL.ExtSupported.maxAnisotropyLevel);
#endif
...
RenderBatch rlLoadRenderBatch(int numBuffers, int bufferElements) {
  ...
  for (int i = 0; i < numBuffers; i++) {
    ...
    glVertexAttribPointer(RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_POSITION], 3, GL_FLOAT, 0,
                          0, 0);
    ...
    glVertexAttribPointer(RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_TEXCOORD01], 2, GL_FLOAT,
                          0, 0, 0);
    ...
    glVertexAttribPointer(RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_COLOR], 4,
...
void rlDrawRenderBatch(RenderBatch* batch) {
  ...
  if (batch->vertexBuffer[batch->currentBuffer].vCounter > 0) {
    ...
    // sizeof(float)*3*4*batch->vertexBuffer[batch->currentBuffer].elementsCount,
    ...
    // glBufferData(GL_ARRAY_BUFFER,
    // sizeof(float)*2*4*batch->vertexBuffer[batch->currentBuffer].elementsCount,
    ...
    // glBufferData(GL_ARRAY_BUFFER,
    // sizeof(float)*4*4*batch->vertexBuffer[batch->currentBuffer].elementsCount,
  ...
  for (int eye = 0; eye < eyesCount; eye++) {
    ...
    if (batch->vertexBuffer[batch->currentBuffer].vCounter > 0) {
      ...
      glUniformMatrix4fv(RLGL.State.currentShader.locs[SHADER_LOC_MATRIX_MVP], 1, false,
                         MatrixToFloat(matMVP));
      ...
      if (RLGL.ExtSupported.vao)
        ...
        glBindBuffer(GL_ARRAY_BUFFER, batch->vertexBuffer[batch->currentBuffer].vboId[0]);
        glVertexAttribPointer(RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_POSITION], 3,
                              GL_FLOAT, 0, 0, 0);
        glEnableVertexAttribArray(RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_POSITION]);

        // Bind vertex attrib: texcoord (shader-location = 1)
        glBindBuffer(GL_ARRAY_BUFFER, batch->vertexBuffer[batch->currentBuffer].vboId[1]);
        glVertexAttribPointer(RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_TEXCOORD01], 2,
                              GL_FLOAT, 0, 0, 0);
        glEnableVertexAttribArray(RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_TEXCOORD01]);
      ...
      glUniform1i(RLGL.State.currentShader.locs[SHADER_LOC_MAP_DIFFUSE],
...
unsigned int rlLoadTexture(void* data, int width, int height, int format, int mipmapCount) {
...
#else
  if ((!RLGL.ExtSupported.texCompDXT) && ((format == PIXELFORMAT_COMPRESSED_DXT1_RGB) ||
  ...
  if ((!RLGL.ExtSupported.texCompETC2) && ((format == PIXELFORMAT_COMPRESSED_ETC2_RGB) ||
                                           (format == PIXELFORMAT_COMPRESSED_ETC2_EAC_RGBA))) {
    TRACELOG(LOG_WARNING, "GL: ETC2 compressed texture format not supported");
    return id;
  }

  if ((!RLGL.ExtSupported.texCompPVRT) && ((format == PIXELFORMAT_COMPRESSED_PVRT_RGB) ||
  ...
  if (id > 0)
    TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Texture loaded successfully (%ix%i | %s | %i mipmaps)", id,
             width, height, rlGetPixelFormatName(format), mipmapCount);
  else
...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer) {
  ...
  if (!useRenderBuffer && RLGL.ExtSupported.texDepth) {
    ...
    TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Depth renderbuffer loaded successfully (%i bits)", id,
...
unsigned int rlLoadTextureCubemap(void* data, int size, int format) {
    ...
    for (unsigned int i = 0; i < 6; i++) {
      if (data == NULL) {
        if (format < PIXELFORMAT_COMPRESSED_DXT1_RGB) {
          ...
          } else if ((format == PIXELFORMAT_UNCOMPRESSED_R32) ||
  ...
  if (id > 0)
    TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Cubemap texture loaded successfully (%ix%i)", id, size,
             size);
  else
...
void rlUpdateTexture(unsigned int id, int offsetX, int offsetY, int width, int height, int format,
  ...
  if ((glInternalFormat != -1) && (format < PIXELFORMAT_COMPRESSED_DXT1_RGB)) {
    ...
    TRACELOG(LOG_WARNING, "TEXTURE: [ID %i] Failed to update for current texture format (%i)", id,
             format);
}
...
void rlGenerateMipmaps(Texture2D* texture) {
  ...
  if (texIsPOT) {
    ...
    if (texture->format == PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) {
      ...
      TRACELOG(LOG_WARNING, "TEXTURE: [ID %i] Mipmaps generated manually on CPU side, total: %i",
               texture->id, texture->mipmaps);
    } else
      TRACELOG(LOG_WARNING,
...
#define MAX(a, b) (((a) > (b)) ? (a) : (b))
    ...
    TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Mipmaps generated automatically, total: %i", texture->id,
             texture->mipmaps);
  }
#endif
  else
...
void* rlReadTexturePixels(Texture2D texture) {
  ...
  if ((glInternalFormat != -1) && (texture.format < PIXELFORMAT_COMPRESSED_DXT1_RGB)) {
    ...
    TRACELOG(LOG_WARNING, "TEXTURE: [ID %i] Data retrieval not suported for pixel format (%i)",
             texture.id, texture.format);
  ...
  pixels = (unsigned char*)RL_MALLOC(
      rlGetPixelDataSize(texture.width, texture.height, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8));
...
int rlGetLocationUniform(unsigned int shaderId, const char* uniformName) {
  ...
  if (location == -1)
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to find shader uniform: %s", shaderId,
             uniformName);
  else
    TRACELOG(LOG_INFO, "SHADER: [ID %i] Shader uniform (%s) set at location: %i", shaderId,
...
int rlGetLocationAttrib(unsigned int shaderId, const char* attribName) {
  ...
  if (location == -1)
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to find shader attribute: %s", shaderId,
             attribName);
  else
    TRACELOG(LOG_INFO, "SHADER: [ID %i] Shader attribute (%s) set at location: %i", shaderId,
             attribName, location);
#endif
  return location;
}
...
Matrix rlGetMatrixTransform(void) {
  ...
  // MatrixMultiply(RLGL.State.stack[i], matStackTransform);
...
static void rlLoadShaderDefault(void) {
  ...
  RLGL.State.defaultVShaderId =
      rlCompileShader(vShaderDefault, GL_VERTEX_SHADER); // Compile default vertex shader
  RLGL.State.defaultFShaderId =
      rlCompileShader(fShaderDefault, GL_FRAGMENT_SHADER); // Compile default fragment shader
  ...
  if (RLGL.State.defaultShader.id > 0) {
    ...
    RLGL.State.defaultShader.locs[SHADER_LOC_MATRIX_MVP] =
        glGetUniformLocation(RLGL.State.defaultShader.id, "mvp");
    ...
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to load default shader",
             RLGL.State.defaultShader.id);
}
...
static void rlUnloadShaderDefault(void) {
  ...
  TRACELOG(LOG_INFO, "SHADER: [ID %i] Default shader unloaded successfully",
           RLGL.State.defaultShader.id);
}
