...
#define BIT_CHECK(a, b) ((a) & (1u << (b)))
  ...
  // This data is automatically allocated to Stack and automatically deallocated at the end of this
  // function
  ...
  int charsHeight = 10;
  int charsDivisor = 1; // Every char is separated from the consecutive by a 1 pixel divisor,
                        // horizontally and vertically

  int charsWidth[224] = {3, 1, 4, 6, 5, 7, 6, 2, 3, 3, 5, 5, 2, 4, 1, 7, 5, 2, 5, 5, 5, 5, 5, 5, 5,
...
extern void UnloadFontDefault(void) {
  for (int i = 0; i < defaultFont.charsCount; i++)
    UnloadImage(defaultFont.chars[i].image);
...
#define FONT_TTF_DEFAULT_CHARS_PADDING 4 // TTF font generation default chars padding
  ...
  if (font.texture.id == 0) {
    ...
    SetTextureFilter(font.texture,
                     TEXTURE_FILTER_POINT); // By default we set point filter (best performance)

  return font;
}

// Load Font from TTF font file with generation parameters
// NOTE: You can pass an array with desired characters, those characters should be available in the
// font if array is NULL, default char set is selected 32..126
Font LoadFontEx(const char* fileName, int fontSize, int* fontChars, int charsCount) {
  ...
  if (fileData != NULL) {
    ...
    font = LoadFontFromMemory(GetFileExtension(fileName), fileData, fileSize, fontSize, fontChars,
                              charsCount);
...
#define COLOR_EQUAL(col1, col2)                                                                    \
  ...
  while ((lineSpacing + lineToRead * (charHeight + lineSpacing)) < image.height) {
    while (
        (xPosToRead < image.width) &&
        !COLOR_EQUAL((pixels[(lineSpacing + (charHeight + lineSpacing) * lineToRead) * image.width +
                             xPosToRead]),
                     key)) {
      ...
      while (!COLOR_EQUAL(
          pixels[(lineSpacing + (charHeight + lineSpacing) * lineToRead) * image.width +
  ...
  for (int i = 0; i < font.charsCount; i++) {
    ...
    // NOTE: On image based fonts (XNA style), character offsets and xAdvance are not required (set
    // to 0)
...
Font LoadFontFromMemory(const char* fileType, const unsigned char* fileData, int dataSize,
  ...
  if (TextIsEqual(fileExtLower, ".ttf") || TextIsEqual(fileExtLower, ".otf")) {
    ...
    font.charsPadding = 0;
    font.chars =
        LoadFontData(fileData, dataSize, font.baseSize, fontChars, font.charsCount, FONT_DEFAULT);

    if (font.chars != NULL) {
      font.charsPadding = FONT_TTF_DEFAULT_CHARS_PADDING;

      Image atlas = GenImageFontAtlas(font.chars, &font.recs, font.charsCount, font.baseSize,
                                      font.charsPadding, 0);
...
CharInfo* LoadFontData(const unsigned char* fileData, int dataSize, int fontSize, int* fontChars,
                       int charsCount, int type) {
...
#define FONT_BITMAP_ALPHA_THRESHOLD 80 // Bitmap (B&W) font generation alpha threshold
  ...
  // NOTE: Loaded information should be enough to generate font image atlas, using any packaging
  // method
  if (fileData != NULL) {
    ...
    if (stbtt_InitFont(&fontInfo, (unsigned char*)fileData, 0)) // Initialize font for data reading
    {
      ...
      for (int i = 0; i < charsCount; i++) {
        ...
        if (type != FONT_SDF)
          chars[i].image.data =
              stbtt_GetCodepointBitmap(&fontInfo, scaleFactor, scaleFactor, ch, &chw, &chh,
        ...
        // NOTE: We create an empty image for space character, it could be further required for
        // atlas packing
        if (ch == 32) {
        ...
        stbtt_GetCodepointBitmapBox(&fontInfo, ch, scaleFactor, scaleFactor, &chX1, &chY1, &chX2,
        &chY2);

        TRACELOGD("FONT: Character box measures: %i, %i, %i, %i", chX1, chY1, chX2 - chX1, chY2 -
        chY1); TRACELOGD("FONT: Character offsetY: %i", (int)((float)ascent*scaleFactor) + chY1);
        */
      }
    } else
...
#if defined(SUPPORT_FILEFORMAT_TTF)
Image GenImageFontAtlas(const CharInfo* chars, Rectangle** charRecs, int charsCount, int fontSize,
  ...
  *charRecs = NULL;
  ...
  atlas.data = (unsigned char*)RL_CALLOC(
      1, atlas.width * atlas.height); // Create a bitmap to store characters (8 bpp)
    ...
    for (int i = 0; i < charsCount; i++) {
      ...
      if (rects[i].was_packed) {
        ...
        for (int y = 0; y < chars[i].image.height; y++) {
          for (int x = 0; x < chars[i].image.width; x++) {
            ((unsigned char*)atlas
                 .data)[(rects[i].y + padding + y) * atlas.width + (rects[i].x + padding + x)] =
                ((unsigned char*)chars[i].image.data)[y * chars[i].image.width + x];
          }
        }
      } else
...
// NOTE: fontSize work like in any drawing program but if fontSize is lower than font-base-size,
// then font-base-size is used NOTE: chars spacing is proportional to fontSize
...
void DrawTextEx(Font font, const char* text, Vector2 position, float fontSize, float spacing,
                Color tint) {
  if (font.texture.id == 0)
...
void DrawTextRecEx(Font font, const char* text, Rectangle rec, float fontSize, float spacing,
  ...
  for (int i = 0, k = 0; i < length; i++, k++) {
    ...
    float glyphWidth = 0;
    if (codepoint != '\n') {
      glyphWidth = (font.chars[index].advanceX == 0) ? font.recs[index].width * scaleFactor
                                                     : font.chars[index].advanceX * scaleFactor;
    ...
    // wordWrap is OFF we don't need the measure state so we go to the drawing state immediately and
    // begin drawing on the next line before we can get outside the container.
    if (state == MEASURE_STATE) {
      // TODO: There are multiple types of spaces in UNICODE, maybe it's a good idea to add support
      // for more Ref: http://jkorpela.fi/chars/spaces.html
      ...
      if (codepoint == '\n') {
        ...
        if ((codepoint != ' ') && (codepoint != '\t')) {
          DrawTextCodepoint(font, codepoint, (Vector2){rec.x + textOffsetX, rec.y + textOffsetY},
...
void DrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint) {
  ...
  Rectangle dstRec = {
      position.x + font.chars[index].offsetX * scaleFactor - (float)font.charsPadding * scaleFactor,
  ...
  // effects
  Rectangle srcRec = {font.recs[index].x - (float)font.charsPadding,
                      font.recs[index].y - (float)font.charsPadding,
                      font.recs[index].width + 2.0f * font.charsPadding,
                      font.recs[index].height + 2.0f * font.charsPadding};
...
Vector2 MeasureTextEx(Font font, const char* text, float fontSize, float spacing) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    // NOTE: normally we exit the decoding sequence as soon as a bad byte is found (and return 0x3f)
    // but we need to draw all of the bad bytes using the '?' symbol so to not skip any we set next
    // = 1
    if (letter == 0x3f)
      next = 1;
    i += next - 1;
    ...
    if (letter != '\n') {
      if (font.chars[index].advanceX != 0)
        ...
        textWidth += (font.recs[index].width + font.chars[index].offsetX);
    } else {
  ...
  Vector2 vec = {0};
  vec.x = tempTextWidth * scaleFactor +
          (float)((tempLen - 1) * spacing); // Adds chars spacing to measure
...
const char** TextSplit(const char* text, char delimiter, int* count) {
  ...
  // delimiter) inserted between strings defined by "delimiter" parameter. No memory is dynamically
  // allocated, all used memory is static... it has some limitations:
...
// When a invalid UTF8 byte is encountered we exit as soon as possible and a '?'(0x3f) codepoint is
...
#define MAX_BUFFER_SIZE 256
  ...
  sscanf(searchPoint, "lineHeight=%i base=%i scaleW=%i scaleH=%i", &fontSize, &base, &imWidth,
         &imHeight);
  fileTextPtr += (lineBytes + 1);
  ...
  if (lastSlash != NULL) {
    ...
    imPath =
        RL_CALLOC(TextLength(fileName) - TextLength(lastSlash) + TextLength(imFileName) + 4, 1);
    memcpy(imPath, fileName, TextLength(fileName) - TextLength(lastSlash) + 1);
    memcpy(imPath + TextLength(fileName) - TextLength(lastSlash) + 1, imFileName,
           TextLength(imFileName));
  } else
    imPath = imFileName;
  ...
  if (font.texture.id == 0) {
    ...
    TRACELOG(LOG_INFO, "FONT: [%s] Font loaded successfully", fileName);
