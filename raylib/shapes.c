...
#define BEZIER_LINE_DIVISIONS 24 // Bezier line divisions
  ...
  for (int i = 1; i <= BEZIER_LINE_DIVISIONS; i++) {
    ...
    current.y =
        EaseCubicInOut((float)i, startPos.y, endPos.y - startPos.y, (float)BEZIER_LINE_DIVISIONS);
...
void DrawLineBezierQuad(Vector2 startPos, Vector2 endPos, Vector2 controlPos, float thick,
                        Color color) {
...
void DrawCircleGradient(int centerX, int centerY, float radius, Color color1, Color color2) {
  ...
  for (int i = 0; i < 360; i += 10) {
    ...
    rlVertex2f((float)centerX + sinf(DEG2RAD * i) * radius,
               (float)centerY + cosf(DEG2RAD * i) * radius);
    rlColor4ub(color2.r, color2.g, color2.b, color2.a);
    rlVertex2f((float)centerX + sinf(DEG2RAD * (i + 10)) * radius,
               (float)centerY + cosf(DEG2RAD * (i + 10)) * radius);
  }
  rlEnd();
}
...
void DrawCircleLines(int centerX, int centerY, float radius, Color color) {
  ...
  for (int i = 0; i < 360; i += 10) {
    ...
    rlVertex2f(centerX + sinf(DEG2RAD * (i + 10)) * radius,
               centerY + cosf(DEG2RAD * (i + 10)) * radius);
  }
  rlEnd();
}
...
void DrawEllipse(int centerX, int centerY, float radiusH, float radiusV, Color color) {
  ...
  for (int i = 0; i < 360; i += 10) {
    rlColor4ub(color.r, color.g, color.b, color.a);
    rlVertex2f((float)centerX, (float)centerY);
    rlVertex2f((float)centerX + sinf(DEG2RAD * i) * radiusH,
               (float)centerY + cosf(DEG2RAD * i) * radiusV);
...
void DrawEllipseLines(int centerX, int centerY, float radiusH, float radiusV, Color color) {
  ...
  for (int i = 0; i < 360; i += 10) {
    ...
    rlVertex2f(centerX + sinf(DEG2RAD * (i + 10)) * radiusH,
               centerY + cosf(DEG2RAD * (i + 10)) * radiusV);
  }
  rlEnd();
}
...
void DrawRing(Vector2 center, float innerRadius, float outerRadius, float startAngle,
  ...
  for (int i = 0; i < segments; i++) {
    ...
    rlTexCoord2f(texShapesRec.x / texShapes.width, texShapesRec.y / texShapes.height);
    rlVertex2f(center.x + sinf(DEG2RAD * angle) * innerRadius,
               center.y + cosf(DEG2RAD * angle) * innerRadius);

    rlTexCoord2f(texShapesRec.x / texShapes.width,
                 (texShapesRec.y + texShapesRec.height) / texShapes.height);
    rlVertex2f(center.x + sinf(DEG2RAD * angle) * outerRadius,
               center.y + cosf(DEG2RAD * angle) * outerRadius);
...
void DrawRectangle(int posX, int posY, int width, int height, Color color) {
  DrawRectangleV((Vector2){(float)posX, (float)posY}, (Vector2){(float)width, (float)height},
                 color);
}
...
void DrawRectangleV(Vector2 position, Vector2 size, Color color) {
  DrawRectanglePro((Rectangle){position.x, position.y, size.x, size.y}, (Vector2){0.0f, 0.0f}, 0.0f,
                   color);
}
...
void DrawRectangleGradientV(int posX, int posY, int width, int height, Color color1, Color color2) {
  DrawRectangleGradientEx((Rectangle){(float)posX, (float)posY, (float)width, (float)height},
...
void DrawRectangleLinesEx(Rectangle rec, float lineThick, Color color) {
  ...
  Rectangle right = {rec.x - lineThick + rec.width, rec.y + lineThick, lineThick,
...
void DrawRectangleRoundedLines(Rectangle rec, float roundness, int segments, float lineThick,
                               Color color) {
  if (lineThick < 0)
    lineThick = 0;
  ...
  if (roundness <= 0.0f) {
    DrawRectangleLinesEx((Rectangle){rec.x - lineThick, rec.y - lineThick,
                                     rec.width + 2 * lineThick, rec.height + 2 * lineThick},
                         lineThick, color);
    return;
  }
  ...
  */
  ...
  if (lineThick > 1) {
    ...
    // Draw all of the 4 corners first: Upper Left Corner, Upper Right Corner, Lower Right Corner,
    // Lower Left Corner
    for (int k = 0; k < 4; ++k) // Hope the compiler is smart enough to unroll this loop
    {
      float angle = angles[k];
    ...
    rlSetTexture(0);
#else
    rlCheckRenderBatchLimit(4 * 6 * segments +
                            4 * 6); // 4 corners with 6(2*3) vertices for each segment + 4
                                    // rectangles with 6 vertices each
...
void DrawPolyLinesEx(Vector2 center, int sides, float radius, float rotation, float lineThick,
  ...
  for (int i = 0; i < sides; i++) {
    ...
    rlTexCoord2f(texShapesRec.x / texShapes.width, texShapesRec.y / texShapes.height);
    rlVertex2f(sinf(DEG2RAD * centralAngle) * innerRadius,
               cosf(DEG2RAD * centralAngle) * innerRadius);
...
// Check the collision between two lines defined by two points each, returns collision point by
// reference
bool CheckCollisionLines(Vector2 startPos1, Vector2 endPos1, Vector2 startPos2, Vector2 endPos2,
                         Vector2* collisionPoint) {
  const float div = (endPos2.y - startPos2.y) * (endPos1.x - startPos1.x) -
  ...
  const float xi =
      ((startPos2.x - endPos2.x) * (startPos1.x * endPos1.y - startPos1.y * endPos1.x) -
       (startPos1.x - endPos1.x) * (startPos2.x * endPos2.y - startPos2.y * endPos2.x)) /
      div;
  const float yi =
      ((startPos2.y - endPos2.y) * (startPos1.x * endPos1.y - startPos1.y * endPos1.x) -
       (startPos1.y - endPos1.y) * (startPos2.x * endPos2.y - startPos2.y * endPos2.x)) /
      div;

  if (xi < fminf(startPos1.x, endPos1.x) || xi > fmaxf(startPos1.x, endPos1.x))
    return false;
  if (xi < fminf(startPos2.x, endPos2.x) || xi > fmaxf(startPos2.x, endPos2.x))
    return false;
  if (yi < fminf(startPos1.y, endPos1.y) || yi > fmaxf(startPos1.y, endPos1.y))
    return false;
  if (yi < fminf(startPos2.y, endPos2.y) || yi > fmaxf(startPos2.y, endPos2.y))
    return false;
...
// Check if point belongs to line created between two points [p1] and [p2] with defined margin in
// pixels [threshold]
