...
static AudioData AUDIO = { // Global AUDIO context
    ...
    // NOTE: Music buffer size is defined by number of samples, independent of sample size and
    // channels number
    // After some math, considering a sampleRate of 48000, a buffer refill rate of 1/60 seconds and
    // a
...
//----------------------------------------------------------------------------------
static void OnLog(ma_context* pContext, ma_device* pDevice, ma_uint32 logLevel,
                  const char* message);
...
static const char*
GetFileExtension(const char* fileName); // Get pointer to extension for a filename string (includes
                                        // the dot: .png)
...
AudioBuffer* LoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate,
                             ma_uint32 sizeInFrames, int usage);
...
void InitAudioDevice(void) {
  ...
  // Keep the device running the whole time. May want to consider doing something a bit smarter and
  // only have the device running while there's at least one sound being played.
  ...
  // Mixing happens on a seperate thread which means we need to synchronize. I'm using a mutex here
  // to make things simple, but may want to look at something a bit smarter later on to keep
  // everything real-time, if that's necessary.
  ...
  for (int i = 0; i < MAX_AUDIO_BUFFER_POOL_CHANNELS; i++) {
    ...
    AUDIO.MultiChannel.pool[i] =
        LoadAudioBuffer(AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate,
                        0, AUDIO_BUFFER_USAGE_STATIC);
  }

  TRACELOG(LOG_INFO, "AUDIO: Device initialized successfully");
  TRACELOG(LOG_INFO, "    > Backend:       miniaudio / %s",
           ma_get_backend_name(AUDIO.System.context.backend));
  TRACELOG(LOG_INFO, "    > Format:        %s -> %s",
           ma_get_format_name(AUDIO.System.device.playback.format),
  ...
  TRACELOG(LOG_INFO, "    > Periods size:  %d",
           AUDIO.System.device.playback.internalPeriodSizeInFrames *
               AUDIO.System.device.playback.internalPeriods);
...
AudioBuffer* LoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate,
  ...
  ma_data_converter_config converterConfig =
      ma_data_converter_config_init(format, AUDIO_DEVICE_FORMAT, channels, AUDIO_DEVICE_CHANNELS,
                                    sampleRate, AUDIO.System.device.sampleRate);
...
void SetAudioBufferPitch(AudioBuffer* buffer, float pitch) {
  if ((buffer != NULL) && (pitch > 0.0f)) {
    ...
    ma_data_converter_set_rate(&buffer->converter, buffer->converter.config.sampleRateIn,
                               outputSampleRate);
...
Wave LoadWaveFromMemory(const char* fileType, const unsigned char* fileData, int dataSize) {
  ...
  if (false) {
    ...
    if (oggData != NULL) {
      ...
      wave.sampleCount = (unsigned int)stb_vorbis_stream_length_in_samples(oggData) *
                         info.channels; // Independent by channel
      wave.data = (short*)RL_MALLOC(wave.sampleCount * sizeof(short));
    ...
    // NOTE: We are forcing conversion to 16bit sample size on reading
    wave.data = drflac_open_memory_and_read_pcm_frames_s16(
        fileData, dataSize, &wave.channels, &wave.sampleRate, &totalFrameCount, NULL);
    wave.sampleSize = 16;
    ...
    wave.data = drmp3_open_memory_and_read_pcm_frames_f32(fileData, dataSize, &config,
  ...
  TRACELOG(LOG_INFO, "WAVE: Data loaded successfully (%i Hz, %i bit, %i channels)", wave.sampleRate,
...
Sound LoadSoundFromWave(Wave wave) {
  ...
  if (wave.data != NULL) {
    ...
    ma_format formatIn =
        ((wave.sampleSize == 8) ? ma_format_u8
    ...
    ma_uint32 frameCount = (ma_uint32)ma_convert_frames(
        NULL, 0, AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate, NULL,
    ...
    AudioBuffer* audioBuffer =
        LoadAudioBuffer(AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate,
                        frameCount, AUDIO_BUFFER_USAGE_STATIC);
    if (audioBuffer == NULL) {
      TRACELOG(LOG_WARNING, "SOUND: Failed to create buffer");
      return sound; // early return to avoid dereferencing the audioBuffer null pointer
    }

    frameCount = (ma_uint32)ma_convert_frames(audioBuffer->data, frameCount, AUDIO_DEVICE_FORMAT,
                                              AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate,
                                              wave.data, frameCountIn, formatIn, wave.channels,
                                              wave.sampleRate);
    if (frameCount == 0)
...
bool ExportWave(Wave wave, const char* fileName) {
  ...
  if (false) {
    ...
    if (success)
      success = (int)drwav_write_pcm_frames(&wav, wave.sampleCount / wave.channels, wave.data);
...
#define TEXT_BYTES_PER_LINE 20
  ...
  bytesCount += sprintf(txtData + bytesCount, "static unsigned char %s_DATA[%i] = { ", varFileName,
                        waveDataSize);
...
void PlaySoundMulti(Sound sound) {
  ...
  if (index == -1) {
    TRACELOG(LOG_WARNING, "SOUND: Buffer pool is already full, count: %i",
...
void StopSoundMulti(void) {
  for (int i = 0; i < MAX_AUDIO_BUFFER_POOL_CHANNELS; i++)
    StopAudioBuffer(AUDIO.MultiChannel.pool[i]);
}
...
void WaveFormat(Wave* wave, int sampleRate, int sampleSize, int channels) {
  ma_format formatIn =
      ((wave->sampleSize == 8) ? ma_format_u8
                               : ((wave->sampleSize == 16) ? ma_format_s16 : ma_format_f32));
  ma_format formatOut =
      ((sampleSize == 8) ? ma_format_u8 : ((sampleSize == 16) ? ma_format_s16 : ma_format_f32));

  ma_uint32 frameCountIn = wave->sampleCount / wave->channels;

  ma_uint32 frameCount =
      (ma_uint32)ma_convert_frames(NULL, 0, formatOut, channels, sampleRate, NULL, frameCountIn,
  ...
  frameCount =
      (ma_uint32)ma_convert_frames(data, frameCount, formatOut, channels, sampleRate, wave->data,
                                   frameCountIn, formatIn, wave->channels, wave->sampleRate);
  if (frameCount == 0) {
...
void WaveCrop(Wave* wave, int initSample, int finalSample) {
  if ((initSample >= 0) && (initSample < finalSample) && (finalSample > 0) &&
      ((unsigned int)finalSample < wave->sampleCount)) {
...
Music LoadMusicStream(const char* fileName) {
  ...
  if (false) {
    ...
    if (music.ctxData != NULL) {
      ...
      // WARNING: It seems this function returns length in frames, not samples, so we multiply by
      // channels
      music.sampleCount =
          (unsigned int)stb_vorbis_stream_length_in_samples((stb_vorbis*)music.ctxData) *
          info.channels;
      music.looping = true; // Looping enabled by default
      musicLoaded = true;
    }
    ...
    if (music.ctxData != NULL) {
      ...
      music.stream =
          LoadAudioStream(ctxFlac->sampleRate, ctxFlac->bitsPerSample, ctxFlac->channels);
...
Music LoadMusicStreamFromMemory(const char* fileType, unsigned char* data, int dataSize) {
  ...
  if (false) {
    ...
    jar_xm_context_t* ctxXm = NULL;
    int result = jar_xm_create_context_safe(&ctxXm, (const char*)data, dataSize,
                                            AUDIO.System.device.sampleRate);
    if (result == 0) // XM AUDIO.System.context created successfully
    {
      music.ctxType = MUSIC_MODULE_XM;
      jar_xm_set_max_loop_count(ctxXm, 0); // Set infinite number of loops
...
void UpdateMusicStream(Music music) {
  ...
  void* pcm =
      RL_CALLOC(subBufferSizeInFrames * music.stream.channels * music.stream.sampleSize / 8, 1);

  int samplesCount = 0; // Total size of data streamed in L+R samples for xm floats, individual L or
                        // R for ogg shorts

  // TODO: Get the sampleLeft using framesProcessed... but first, get total frames processed
  // correctly...
  ...
  int sampleLeft =
      music.sampleCount - (music.stream.buffer->framesProcessed * music.stream.channels);
  ...
  while (IsAudioStreamProcessed(music.stream)) {
    ...
    case MUSIC_AUDIO_WAV: {
      ...
      if (music.stream.sampleSize == 16)
        drwav_read_pcm_frames_s16((drwav*)music.ctxData, samplesCount / music.stream.channels,
                                  (short*)pcm);
      else if (music.stream.sampleSize == 32)
        drwav_read_pcm_frames_f32((drwav*)music.ctxData, samplesCount / music.stream.channels,
                                  (float*)pcm);
    ...
    case MUSIC_AUDIO_OGG: {
      ...
      stb_vorbis_get_samples_short_interleaved((stb_vorbis*)music.ctxData, music.stream.channels,
                                               (short*)pcm, samplesCount);
...
#if defined(SUPPORT_FILEFORMAT_MP3)
    case MUSIC_AUDIO_MP3: {
      // NOTE: samplesCount, actually refers to framesCount and returns the number of frames
      // processed
      drmp3_read_pcm_frames_f32((drmp3*)music.ctxData, samplesCount / music.stream.channels,
                                (float*)pcm);
    ...
    case MUSIC_MODULE_XM: {
      ...
      if (AUDIO_DEVICE_FORMAT == ma_format_f32)
        ...
        jar_xm_generate_samples_16bit((jar_xm_context_t*)music.ctxData, (short*)pcm,
                                      samplesCount / 2);
      else if (AUDIO_DEVICE_FORMAT == ma_format_u8)
        jar_xm_generate_samples_8bit((jar_xm_context_t*)music.ctxData, (char*)pcm,
                                     samplesCount / 2);
...
#if defined(SUPPORT_FILEFORMAT_MOD)
    case MUSIC_MODULE_MOD: {
      // NOTE: 3rd parameter (nbsample) specify the number of stereo 16bits samples you want, so
      // sampleCount/2
...
AudioStream LoadAudioStream(unsigned int sampleRate, unsigned int sampleSize,
  ...
  ma_format formatIn =
      ((stream.sampleSize == 8) ? ma_format_u8
                                : ((stream.sampleSize == 16) ? ma_format_s16 : ma_format_f32));

  // The size of a streaming buffer must be at least double the size of a period
  unsigned int periodSize = AUDIO.System.device.playback.internalPeriodSizeInFrames;

  // If the buffer is not set, compute one that would give us a buffer good enough for a decent
  // frame rate
  unsigned int subBufferSize = (AUDIO.Buffer.defaultSize == 0) ? AUDIO.System.device.sampleRate / 30
                                                               : AUDIO.Buffer.defaultSize;

  if (subBufferSize < periodSize)
    subBufferSize = periodSize;

  // Create a double audio buffer of defined size
  stream.buffer = LoadAudioBuffer(formatIn, stream.channels, stream.sampleRate, subBufferSize * 2,
                                  AUDIO_BUFFER_USAGE_STREAM);

  if (stream.buffer != NULL) {
    stream.buffer->looping = true; // Always loop for streaming buffers
    TRACELOG(LOG_INFO, "STREAM: Initialized successfully (%i Hz, %i bit, %s)", stream.sampleRate,
             stream.sampleSize, (stream.channels == 1) ? "Mono" : "Stereo");
  } else
      ...
      if (subBufferSizeInFrames >= (ma_uint32)samplesCount / stream.channels) {
        ...
        if (leftoverFrameCount > 0)
          memset(subBuffer + bytesToWrite, 0,
                 leftoverFrameCount * stream.channels * (stream.sampleSize / 8));
...
static ma_uint32 ReadAudioBufferFramesInInternalFormat(AudioBuffer* audioBuffer, void* framesOut,
  ...
  ma_uint32 frameSizeInBytes = ma_get_bytes_per_frame(audioBuffer->converter.config.formatIn,
                                                      audioBuffer->converter.config.channelsIn);

  // Fill out every frame until we find a buffer that's marked as processed. Then fill the remainder
  // with 0
  ...
  while (1) {
    ...
    audioBuffer->frameCursorPos =
        (audioBuffer->frameCursorPos + framesToRead) % audioBuffer->sizeInFrames;
  ...
  ma_uint32 totalFramesRemaining = (frameCount - framesRead);
  if (totalFramesRemaining > 0) {
    memset((unsigned char*)framesOut + (framesRead * frameSizeInBytes), 0,
           totalFramesRemaining * frameSizeInBytes);

    // For static buffers we can fill the remaining frames with silence for safety, but we don't
    // want to report those frames as "read". The reason for this is that the caller uses the return
    // value to know whether or not a non-looping sound has finished playback.
    if (audioBuffer->usage != AUDIO_BUFFER_USAGE_STATIC)
...
// Reads audio data from an AudioBuffer object in device format. Returned data will be in a format
// appropriate for mixing.
static ma_uint32 ReadAudioBufferFramesInMixingFormat(AudioBuffer* audioBuffer, float* framesOut,
                                                     ma_uint32 frameCount) {
  // What's going on here is that we're continuously converting data from the AudioBuffer's internal
  // format to the mixing format, which should be defined by the output format of the data
  // converter. We do this until frameCount frames have been output. The important detail to
  // remember here is that we never, ever attempt to read more input data than is required for the
  // specified number of output frames. This can be achieved with
  // ma_data_converter_get_required_input_frame_count().
  ma_uint8 inputBuffer[4096];
  ma_uint32 inputBufferFrameCap =
      sizeof(inputBuffer) / ma_get_bytes_per_frame(audioBuffer->converter.config.formatIn,
                                                   audioBuffer->converter.config.channelsIn);
  ...
  while (totalOutputFramesProcessed < frameCount) {
    ...
    ma_uint64 inputFramesToProcessThisIteration = ma_data_converter_get_required_input_frame_count(
        &audioBuffer->converter, outputFramesToProcessThisIteration);
    if (inputFramesToProcessThisIteration > inputBufferFrameCap) {
      inputFramesToProcessThisIteration = inputBufferFrameCap;
    }

    float* runningFramesOut =
        framesOut + (totalOutputFramesProcessed * audioBuffer->converter.config.channelsOut);
    ...
    ma_data_converter_process_pcm_frames(&audioBuffer->converter, inputBuffer,
                                         &inputFramesProcessedThisIteration, runningFramesOut,
    ...
    /* This should never be hit, but will add it here for safety. Ensures we get out of the loop
     * when no input nor output frames are processed. */
...
static void OnSendAudioDataToDevice(ma_device* pDevice, void* pFramesOut, const void* pFramesInput,
  ...
  // This is unlikely to be necessary for this project, but may want to consider how you might want
  // to avoid this
  ma_mutex_lock(&AUDIO.System.lock);
  {
    for (AudioBuffer* audioBuffer = AUDIO.Buffer.first; audioBuffer != NULL;
         audioBuffer = audioBuffer->next) {
      ...
      while (1) {
        ...
        while (framesToRead > 0) {
          ...
          if (framesToReadRightNow >
              sizeof(tempBuffer) / sizeof(tempBuffer[0]) / AUDIO_DEVICE_CHANNELS) {
            framesToReadRightNow =
                sizeof(tempBuffer) / sizeof(tempBuffer[0]) / AUDIO_DEVICE_CHANNELS;
          }

          ma_uint32 framesJustRead =
              ReadAudioBufferFramesInMixingFormat(audioBuffer, tempBuffer, framesToReadRightNow);
          if (framesJustRead > 0) {
            float* framesOut =
                (float*)pFramesOut + (framesRead * AUDIO.System.device.playback.channels);
...
// This is the main mixing function. Mixing is pretty simple in this project - it's just an
// accumulation. NOTE: framesOut is both an input and an output. It will be initially filled with
// zeros outside of this function.
static void MixAudioFrames(float* framesOut, const float* framesIn, ma_uint32 frameCount,
                           float localVolume) {
