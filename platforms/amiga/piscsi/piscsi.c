...
static const char* piscsi_cpu_type_name(unsigned int cpu_type) {
  ...
  static const char* cpu_names[] = {
      "invalid", "68000",   "68010",   "68EC020", "68020",    "68EC030",
...
void piscsi_init(struct emulator_config* local_cfg) {
...

  {
    ...
    char z2_desc[64];
    if (z2_idx >= 0) {
      snprintf(z2_desc, sizeof(z2_desc), "0x%08lX+0x%X", (unsigned long)use_cfg->map_offset[z2_idx],
               use_cfg->map_size[z2_idx]);
    } else {
  ...
  if (piscsi_rom_ptr == NULL) {
    ...
    if (in == NULL) {
      LOG_ERROR(
          "[PISCSI][INIT] Boot ROM open failed: ./platforms/amiga/piscsi/piscsi.rom (errno=%d)\n",
          errno);
...
next_partition:;
  ...
  LOG_INFO("[PISCSI][PART] Unit %d: partition %d name=%s len=%d\n", unit, cur_partition,
           pb->pb_DriveName + 1, pb->pb_DriveName[0]);
  DEBUG("[PISCSI][PART] Unit %d: checksum=0x%.8X hostid=%d\n", unit, BE(pb->pb_ChkSum),
        BE(pb->pb_HostID));
  DEBUG("[PISCSI][PART] Unit %d: flags=%d (0x%.8X) devflags=%d (0x%.8X)\n", unit, BE(pb->pb_Flags),
        BE(pb->pb_Flags), BE(pb->pb_DevFlags), BE(pb->pb_DevFlags));
...
void piscsi_find_filesystems(struct piscsi_dev* d) {
  ...
  while (BE(fhb->fhb_ID) == FS_IDENTIFIER) {
    ...
    if (load_lseg(d->fd, &filesystems[piscsi_num_fs].binary_data,
                  &filesystems[piscsi_num_fs].h_info, filesystems[piscsi_num_fs].relocs,
                  d->block_size) != -1) {
      filesystems[piscsi_num_fs].FS_ID = fhb->fhb_DosType;
      filesystems[piscsi_num_fs].fhb = fhb;
      LOG_INFO("[PISCSI][FS] Unit %d: loaded filesystem #%d %c%c%c/%d\n", unit, piscsi_num_fs + 1,
               dosID[0], dosID[1], dosID[2], dosID[3]);
      {
        ...
        if (save_fs == NULL) {
          ...
          if (save_fs != NULL) {
            fwrite(filesystems[piscsi_num_fs].binary_data,
                   filesystems[piscsi_num_fs].h_info.byte_size, 1, save_fs);
            fclose(save_fs);
            LOG_INFO("[PISCSI][FS] Unit %d: saved filesystem %c%c%c/%d to ./data/fs.\n", unit,
                     dosID[0], dosID[1], dosID[2], dosID[3]);
          } else {
            LOG_WARN("[PISCSI][FS] Unit %d: failed to save filesystem to ./data/fs (permission "
                     "issue?).\n",
                     unit);
          }
        } else {
...
void piscsi_map_drive(char* filename, uint8_t index) {
  ...
  hdfID[4] = '\0';
  if (strcmp(hdfID, "DOS") == 0 || strcmp(hdfID, "PFS") == 0 || strcmp(hdfID, "PDS") == 0 ||
      strcmp(hdfID, "SFS") == 0) {
    LOG_WARN("[PISCSI][INIT] %s looks like a UAE single-partition hardfile (unsupported).\n",
             filename);
    LOG_WARN("[PISCSI][INIT] Use full-drive RDB images for PiSCSI. See "
             "platforms/amiga/piscsi/readme.md.\n");
    LOG_INFO("[PISCSI][INIT] If this is an empty placeholder for on-Amiga partitioning, ignore "
             "this warning.\n");
  }
...
int piscsi_validate_hdf(struct piscsi_dev* d, char* filename) {
  ...
  if (d - devs == 0) { // This is unit 0 (DH0)
    ...
    uint32_t dos_sig =
        (boot_block[0] << 24) | (boot_block[1] << 16) | (boot_block[2] << 8) | boot_block[3];
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (bytes_read < 512) {
      LOG_ERROR(
          "[PISCSI][SELFTEST] Cannot read test block at offset %lld from %s (got %zd bytes)\n",
...
void print_piscsi_debug_message(int index) {
  ...
  case DBG_OPENDEV:
    if (piscsi_dbg[0] != 255) {
      DEBUG("[PISCSI][DRV] OpenDevice unit=%d flags=%d (0x%.2X) err=%d\n", piscsi_dbg[0],
            piscsi_dbg[1], piscsi_dbg[1], piscsi_dbg[2]);
    }
    break;
  case DBG_CLEANUP:
  ...
  case DBG_BEGINIO:
    DEBUG("[PISCSI][IO] BeginIO cmd=%d (%s) flags=0x%X quick=%d\n", piscsi_dbg[0],
          io_cmd_name(piscsi_dbg[0]), piscsi_dbg[1], piscsi_dbg[2]);
    break;
  case DBG_ABORTIO:
  ...
  case DBG_SCSICMD:
    DEBUG("[PISCSI][SCSI] cmd=0x%.2X (%s) cdb_len=%d bytes: %.2X %.2X %.2X\n", piscsi_dbg[1],
          scsi_cmd_name(piscsi_dbg[1]), piscsi_dbg[4], piscsi_dbg[1], piscsi_dbg[2], piscsi_dbg[3]);
    break;
...
void piscsi_debugme(uint32_t index) {
  ...
  case 35:
    ...
    break;
  case 36:
    DEBUG("[PISCSI][DBGME] Debug pointers: %.8X %.8X %.8X %.8X\n", piscsi_u32[0], piscsi_u32[1],
          piscsi_u32[2], piscsi_u32[3]);
    break;
  default:
...
void handle_piscsi_write(uint32_t addr, uint32_t val, uint8_t type) {
  ...
  case PISCSI_CMD_READBYTES:
    ...
    if (cmd == PISCSI_CMD_READBYTES) {
      ...
      d->lba = block;
      DEBUG("[PISCSI][IO] Unit:%d READBYTES offset=0x%X len=%d LBA=0x%X file_offset=0x%X "
            "dst=0x%.8X\n",
      ...
      d->lba = block;
      DEBUG("[PISCSI][IO] Unit:%d READ64 offset=0x%llX len=%d LBA=0x%X file_offset=0x%llX "
            "dst=0x%.8X\n",
            val, (unsigned long long)src, piscsi_u32[1], block, (unsigned long long)src,
            piscsi_u32[2]);
  ...
  case PISCSI_CMD_WRITEBYTES:
    ...
    if (cmd == PISCSI_CMD_WRITEBYTES) {
      ...
      uint32_t block = src / d->block_size;
      d->lba = block;
      DEBUG("[PISCSI][IO] Unit:%d WRITEBYTES offset=0x%X len=%d LBA=0x%X file_offset=0x%X "
            "src=0x%.8X\n",
            val, src, piscsi_u32[1], block, src, piscsi_u32[2]);
      lseek(d->fd, src, SEEK_SET);
      ...
      d->lba = block;
      DEBUG(
          "[PISCSI][IO] Unit:%d WRITE offset=0x%X len=%d LBA=0x%X file_offset=0x%llX src=0x%.8X\n",
          val, block, piscsi_u32[1], block, (unsigned long long)file_offset, piscsi_u32[2]);
      lseek(d->fd, file_offset, SEEK_SET);
    } else {
      uint64_t src = ((uint64_t)piscsi_u32[3] << 32) | piscsi_u32[0];
      uint32_t block = (uint32_t)(src / d->block_size);
      d->lba = block;
      DEBUG("[PISCSI][IO] Unit:%d WRITE64 offset=0x%llX len=%d LBA=0x%X file_offset=0x%llX "
            "src=0x%.8X\n",
            val, (unsigned long long)src, piscsi_u32[1], block, (unsigned long long)src,
            piscsi_u32[2]);
      lseek64(d->fd, src, SEEK_SET);
...
#define PUTNODELONG(val)                                                                           \
  *(uint32_t*)&dst_data[p_offs] = htobe32(val);                                                    \
  p_offs += 4;
#define PUTNODELONGBE(val)                                                                         \
  *(uint32_t*)&dst_data[p_offs] = val;                                                             \
  p_offs += 4;
        ...
        if (devs[i].num_partitions) {
          ...
          for (uint32_t j = 0; j < devs[i].num_partitions; j++) {
            ...
            DEBUG("[PISCSI][ROM] DOSNode: lowcyl=%d highcyl=%d prio=%d\n", BE(dat->lowcyl),
                  BE(dat->highcyl), BE(dat->priority));
  ...
  case PISCSI_CMD_COPYFS:
    ...
    if (r != -1) {
      ...
      memcpy(cfg->map_data[r] + addr, filesystems[rom_cur_fs].binary_data,
             filesystems[rom_cur_fs].h_info.byte_size);
      filesystems[rom_cur_fs].h_info.base_offset = piscsi_u32[2];
      reloc_hunks(filesystems[rom_cur_fs].relocs, cfg->map_data[r] + addr,
                  &filesystems[rom_cur_fs].h_info);
  ...
  case PISCSI_CMD_SETFSH: {
    ...
    if (r != -1) {
      ...
      for (i = 0; i < piscsi_num_fs; i++) {
        if (rom_partition_dostype[rom_cur_partition] == filesystems[i].FS_ID) {
          node->dn_SegList =
              htobe32((((filesystems[i].handler) + filesystems[i].h_info.header_size) >> 2));
    ...
    fs_found:;
      ...
      DEBUG("[PISCSI][ROM] FS handler: handler=%d stack=%d\n", BE(node->dn_Handler),
            BE(node->dn_StackSize));
      DEBUG("[PISCSI][ROM] FS handler: prio=%d startup=%d (0x%.8X)\n", BE(node->dn_Priority),
            BE(node->dn_Startup), BE(node->dn_Startup));
...
uint32_t handle_piscsi_read(uint32_t addr, uint8_t type) {
  ...
  if ((addr & 0xFFFF) >= PISCSI_CMD_ROM) {
    ...
    if (romoffs < (piscsi_rom_size + PIB)) {
      // DEBUG("[PISCSI] %s read from Boot ROM @$%.4X (%.8X): ", op_type_names[type], romoffs,
      // addr);
      uint32_t v = 0;
      switch (type) {
      case OP_TYPE_BYTE:
  ...
  case PISCSI_CMD_BLOCKSIZE:
    DEBUG("[PISCSI][RDB] Get block size unit=%d: %d\n", piscsi_cur_drive,
          devs[piscsi_cur_drive].block_size);
